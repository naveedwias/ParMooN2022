...
{
  values[0] = std::sin(Pi*x)*std::sin(2*Pi*y);
  values[1] = Pi*std::cos(Pi*x)*std::sin(2*Pi*y);
  values[2] = 2*Pi*std::sin(Pi*x)*std::cos(2*Pi*y);
  values[3] = 0;
}
        ...
        double **parameters, double **coeffs)
{
  ...
  double lambda = a*std::sqrt(rho*g/sigma);
  double Si = 4*Pi*0.1*chi*chi*H0*H0/(2*std::sqrt(sigma*g*rho));

  if(first)
  {
    Output::print("lambda: ", lambda);
    Output::print("Si: ", Si);
        ...
        double **parameters, double **coeffs)
{
  ...
  double lambda = a*std::sqrt(rho*g/sigma);
  double Si = 4*Pi*0.1*Ms*Ms/(3*chi*std::sqrt(sigma*g*rho));
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    H = std::sqrt(param[2]*param[2] + param[3]*param[3] + param[4]*param[4]);
    arg = gamma*H;
    Im = std::log( std::sinh(arg) / arg );

    L = 1/std::tanh(arg) - 1/arg;

    phin = (param[2]*param[5] + param[3]*param[6] + param[4]*param[7])/H;

    coeff[4] = 4*Pi*0.1*a/sigma
                 *Ms * (  H0*(H-HT*std::log((H+HT)/HT) )
...
{
  double n1, n2, n3;
  out[0] = 1/std::sqrt(1+in[2]*in[2]+in[3]*in[3]); // 1/std::sqrt(1+|grad u|^2)
}
