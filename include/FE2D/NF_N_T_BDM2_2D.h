...
static double NF_N_T_BDM2_2D_T[4] = 
 {-std::sqrt(3./7. + (2./7.)*std::sqrt(6./5.)), -std::sqrt(3./7. - (2./7.)*std::sqrt(6./5.)),
   std::sqrt(3./7. - (2./7.)*std::sqrt(6./5.)),  std::sqrt(3./7. + (2./7.)*std::sqrt(6./5.)) };

// the points on the line [0,1]
static double NF_N_T_BDM2_2D_q[4] = 
{ 0.5*NF_N_T_BDM2_2D_T[0] + 0.5, 0.5*NF_N_T_BDM2_2D_T[1] + 0.5,
  0.5*NF_N_T_BDM2_2D_T[2] + 0.5, 0.5*NF_N_T_BDM2_2D_T[3] + 0.5 };
// weights for the 1D Gauss quadrature with four points
// { 0.34785, 0.65215, 0.65215, 0.34785 }
static double NF_N_T_BDM2_2D_w[4] =
{ (18. - std::sqrt(30.)) / 72., (18. + std::sqrt(30.)) / 72.,
  (18. + std::sqrt(30.)) / 72., (18. - std::sqrt(30.)) / 72. }; 

// P_2(x) with P_2 being the second Legendre polynomial and x the quad points 
// from above
static double NF_N_T_BDM2_2D_p2[4] = 
  { 0.5*(3*NF_N_T_BDM2_2D_T[0]*NF_N_T_BDM2_2D_T[0] - 1.),
    0.5*(3*NF_N_T_BDM2_2D_T[1]*NF_N_T_BDM2_2D_T[1] - 1.),
    0.5*(3*NF_N_T_BDM2_2D_T[2]*NF_N_T_BDM2_2D_T[2] - 1.),
    0.5*(3*NF_N_T_BDM2_2D_T[3]*NF_N_T_BDM2_2D_T[3] - 1.) };

// four point Gauss quadrature for edge dofs, 19-point formula for inner dofs
static double NF_N_T_BDM2_2D_Xi[] = 
{ NF_N_T_BDM2_2D_q[0], NF_N_T_BDM2_2D_q[1] , NF_N_T_BDM2_2D_q[2], 
  NF_N_T_BDM2_2D_q[3],
  NF_N_T_BDM2_2D_q[3], NF_N_T_BDM2_2D_q[2], NF_N_T_BDM2_2D_q[1], 
  NF_N_T_BDM2_2D_q[0],
  0, 0, 0, 0,
  ...
  0.0368384120547362836348175987833851,
  0.221962989160765695675102527693192 };
static double NF_N_T_BDM2_2D_Eta[] = 
{ 0, 0, 0, 0,
  NF_N_T_BDM2_2D_q[0], NF_N_T_BDM2_2D_q[1] , NF_N_T_BDM2_2D_q[2], 
  NF_N_T_BDM2_2D_q[3],
  NF_N_T_BDM2_2D_q[3], NF_N_T_BDM2_2D_q[2], NF_N_T_BDM2_2D_q[1], 
...
void NF_N_T_BDM2_2D_EvalAll(const TCollection *, const TBaseCell *Cell,
                            const double *PointValues, double *Functionals)
{
  ...
  // short names
  const double * ew = NF_N_T_BDM2_2D_w;
  const double * iw = NF_N_T_BDM2_2D_iw;
  const double * p = NF_N_T_BDM2_2D_T;
  const double * p2= NF_N_T_BDM2_2D_p2;
  const double * xi = NF_N_T_BDM2_2D_Xi+12; // inner points
  const double *eta = NF_N_T_BDM2_2D_Eta+12; // inner pionts
  ...
  if(Cell == nullptr)
  {
    ...
    for(unsigned int i = 0; i < 19; ++i)
    {
      ...
      // curl of the bubble function x*y*(1-x-y)
      double cbx = xi[i] - xi[i]*xi[i] - 2*xi[i]*eta[i];
      double cby = -eta[i] + 2*xi[i]*eta[i] + eta[i]*eta[i];
      Functionals[11] += (PointValues[12+i]*cbx + PointValues[43+i]*cby)* iw[i];
    }
  }
  else // on a real cell
  {
    ...
    for(unsigned int i = 0; i < 19; ++i) // loop over inner quadrature points
    {
      ...
      double uxi[3] = {  1,0,  x-x*x-2*x*y };
      double ueta[3] = { 0,1,  -y+2*x*y+y*y };
      for(unsigned int d = 0; d < 3; ++d) // inner dofs
      {
...
void NF_N_T_BDM2_2D_EvalEdge(const TCollection *, const TBaseCell *Cell, int Joint,
                            const double *PointValues, double *Functionals)
{
  ...
  Functionals[0] = ( NF_N_T_BDM2_2D_w[0]*PointValues[0]
                    +NF_N_T_BDM2_2D_w[1]*PointValues[1]
                    +NF_N_T_BDM2_2D_w[2]*PointValues[2]
                    +NF_N_T_BDM2_2D_w[3]*PointValues[3] )*l;
  Functionals[1] = ( NF_N_T_BDM2_2D_w[0]*NF_N_T_BDM2_2D_T[0]*PointValues[0]
                    +NF_N_T_BDM2_2D_w[1]*NF_N_T_BDM2_2D_T[1]*PointValues[1]
                    +NF_N_T_BDM2_2D_w[2]*NF_N_T_BDM2_2D_T[2]*PointValues[2]
                    +NF_N_T_BDM2_2D_w[3]*NF_N_T_BDM2_2D_T[3]*PointValues[3] )*l;
  Functionals[2] = ( NF_N_T_BDM2_2D_w[0]*NF_N_T_BDM2_2D_p2[0]*PointValues[0]
                    +NF_N_T_BDM2_2D_w[1]*NF_N_T_BDM2_2D_p2[1]*PointValues[1]
                    +NF_N_T_BDM2_2D_w[2]*NF_N_T_BDM2_2D_p2[2]*PointValues[2]
                    +NF_N_T_BDM2_2D_w[3]*NF_N_T_BDM2_2D_p2[3]*PointValues[3])*l;
}
