...
static double NF_N_T_BDM3_2D_T[5] = 
...
static double NF_N_T_BDM3_2D_q[5] = 
{ 0.5*NF_N_T_BDM3_2D_T[0] + 0.5, 0.5*NF_N_T_BDM3_2D_T[1] + 0.5,
  0.5*NF_N_T_BDM3_2D_T[2] + 0.5, 0.5*NF_N_T_BDM3_2D_T[3] + 0.5,
  0.5*NF_N_T_BDM3_2D_T[4] + 0.5
};
// weights for the 1D Gauss quadrature with four points
// { 0.34785, 0.65215, 0.65215, 0.34785 }
static double NF_N_T_BDM3_2D_w[5] =
...
// from above
static double NF_N_T_BDM3_2D_p2[5] = 
 { 0.5*(3*NF_N_T_BDM3_2D_T[0]*NF_N_T_BDM3_2D_T[0] - 1.), 
   0.5*(3*NF_N_T_BDM3_2D_T[1]*NF_N_T_BDM3_2D_T[1] - 1.),
   0.5*(3*NF_N_T_BDM3_2D_T[2]*NF_N_T_BDM3_2D_T[2] - 1.),
   0.5*(3*NF_N_T_BDM3_2D_T[3]*NF_N_T_BDM3_2D_T[3] - 1.),
   0.5*(3*NF_N_T_BDM3_2D_T[4]*NF_N_T_BDM3_2D_T[4] - 1.) };
// P_3(x) with P_3 being the third Legendre polynomial and x the Gauss 
// quadrature points from above
static double NF_N_T_BDM3_2D_p3[5] = 
{ 0.5*( 5.*NF_N_T_BDM3_2D_T[0]*NF_N_T_BDM3_2D_T[0]*NF_N_T_BDM3_2D_T[0] 
       -3.*NF_N_T_BDM3_2D_T[0]),
  0.5*( 5.*NF_N_T_BDM3_2D_T[1]*NF_N_T_BDM3_2D_T[1]*NF_N_T_BDM3_2D_T[1] 
       -3.*NF_N_T_BDM3_2D_T[1]),
  0.5*( 5.*NF_N_T_BDM3_2D_T[2]*NF_N_T_BDM3_2D_T[2]*NF_N_T_BDM3_2D_T[2] 
       -3.*NF_N_T_BDM3_2D_T[2]),
  0.5*( 5.*NF_N_T_BDM3_2D_T[3]*NF_N_T_BDM3_2D_T[3]*NF_N_T_BDM3_2D_T[3] 
       -3.*NF_N_T_BDM3_2D_T[3]),
  0.5*( 5.*NF_N_T_BDM3_2D_T[4]*NF_N_T_BDM3_2D_T[4]*NF_N_T_BDM3_2D_T[4] 
       -3.*NF_N_T_BDM3_2D_T[4]),
};

// four point Gauss quadrature for edge dofs, 19-point formula for inner dofs
static double NF_N_T_BDM3_2D_Xi[] = 
{ NF_N_T_BDM3_2D_q[0], NF_N_T_BDM3_2D_q[1], NF_N_T_BDM3_2D_q[2], 
  NF_N_T_BDM3_2D_q[3], NF_N_T_BDM3_2D_q[4],
  NF_N_T_BDM3_2D_q[4], NF_N_T_BDM3_2D_q[3], NF_N_T_BDM3_2D_q[2], 
  NF_N_T_BDM3_2D_q[1], NF_N_T_BDM3_2D_q[0],
  0, 0, 0, 0, 0,
...
void NF_N_T_BDM3_2D_EvalAll(const TCollection *, const TBaseCell *Cell,
                            const double *PointValues, double *Functionals)
{
  ...
  // short names
  const double * q = NF_N_T_BDM3_2D_T;
  const double * ew = NF_N_T_BDM3_2D_w;
  const double * p2 = NF_N_T_BDM3_2D_p2;
  const double * p3 = NF_N_T_BDM3_2D_p3;
  const double * iw = NF_N_T_BDM3_2D_iw;
  const double * xi = NF_N_T_BDM3_2D_Xi+15; // +15 to skip the points on edges
  const double * et = NF_N_T_BDM3_2D_Eta+15; // +15 to skip the points on edges
  ...
  if(Cell == nullptr)
  {
    ...
    for(unsigned int i = 0; i < 19; ++i)
    {
      ...
      Functionals[14] += PointValues[15+i] * iw[i] * 2*xi[i];
      Functionals[15] += PointValues[49+i] * iw[i] * 2*et[i];
  ...
  else // on a real cell
  {
    ...
    // int_cell v . (0 2y)^T
    // int_cell v . (y x)^T
    // int_cell v . curl(b)^T  with b(x,y) = x*y*(1-x-y)
    // int_cell v . curl(b*x)^T  with b(x,y) = x*y*(1-x-y)
    // int_cell v . curl(b*y)^T  with b(x,y) = x*y*(1-x-y)
    for(unsigned int i = 0; i < 19; ++i) // loop over inner quadrature points
    {
      double x = xi[i], y = et[i];
      double uxi[12] = {  1,0,  2*x,0,    y,  x-x*x-2*x*y,  x*x-x*x*x-2*x*x*y,  2*x*y-2*x*x*y-3*x*y*y };
      double ueta[12] = { 0,1,  0,2*y,  x,  -y+2*x*y+y*y, -2*x*y+3*x*x*y+2*x*y*y,  -y*y+2*x*y*y+y*y*y };
      for(unsigned int d = 0; d < 8; ++d) // inner dofs
      {
...
void NF_N_T_BDM3_2D_EvalEdge(const TCollection *, const TBaseCell *Cell, int Joint,
                            const double *PointValues, double *Functionals)
{
  ...
  for(unsigned int i = 0; i < 5; ++i)
  {
    ...
    Functionals[0] += pv * NF_N_T_BDM3_2D_w[i];
    Functionals[1] += pv * NF_N_T_BDM3_2D_w[i] * NF_N_T_BDM3_2D_T[i];
    Functionals[2] += pv * NF_N_T_BDM3_2D_w[i] * NF_N_T_BDM3_2D_p2[i];
    Functionals[3] += pv * NF_N_T_BDM3_2D_w[i] * NF_N_T_BDM3_2D_p3[i];
  }
