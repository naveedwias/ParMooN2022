// ======================================================================
// ThreeInteriorLayers_inlet_M_bump
// ======================================================================
...
void ExampleFile()
{
  Output::print<1>("Example: ThreeInteriorLayers_inlet_M_bump.h");
}
// exact solution
...
void ComputeOutflowBoundary(const TFEFunction2D *ufct)
{
  ...
  Output::print("dof ",ufct->GetLength(), " extr1 ",loc_ext[0], " ", loc_ext[3], 
   ...
   int no_extr = 0; 
   double profile_width_start, profile_width, bump_start;
   ...
   for (i=0;i<bound_points; i++)
   {
     ...
     //       " " <<  uval[i] << endl);
     //
    if ((y_coord[i] > 0.72) && (y_coord[i] < 0.86) && (uval[i-1] <0.1) && (uval[i] >=0.1))
    {
      // linear interpolation 
      bump_start = y_coord[i-1] + (0.1-uval[i-1])*(y_coord[i]-y_coord[i-1])/(uval[i]-uval[i-1]);
      Output::print("dof " ,ufct->GetLength() ,  " bump starts at ", bump_start , " length ",  1-bump_start);
    }
    if (y_coord[i] < 0.7)
    {
       if (uval[i-1] <0.1 && uval[i] >=0.1)
          // linear interpolation 
          profile_width_start = y_coord[i-1] + (0.1-uval[i-1])*(y_coord[i]-y_coord[i-1])/(uval[i]-uval[i-1]);
       if (uval[i-1] >=0.1 && uval[i] <0.1)
       {
          // linear interpolation     
          profile_width = y_coord[i-1] + (0.1-uval[i-1])*(y_coord[i]-y_coord[i-1])/(uval[i]-uval[i-1]);
          profile_width = profile_width - profile_width_start;
          Output::print("dof " ,ufct->GetLength() ,  " profile width "  , profile_width);
       }
    }
  ...
  Output::print("dof ", ufct->GetLength() , " u(0,1) = ", uval[bound_points-1]);
...
void three_interior_layers_inlet_M_bump_postprocessing(ConvectionDiffusion<2> & cd2d)
{
