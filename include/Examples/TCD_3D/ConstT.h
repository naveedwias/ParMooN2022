...
//   z = 2.*z-1.;
  ...
  values[0] = (64.*T_int/(PI*PI*PI))*(std::exp(-3.*PI*PI*t/4.))*std::cos(PI*x/2.)*std::cos(PI*y/2.)*std::cos(PI*z/2.);
  values[1] = 0;
  values[2] = 0;
  values[3] = 0;
  values[4] = 0;
...
//       n = (double)j;
//       l = (double)k;
//       a =(64.*(T_int - T_w)/(PI*PI*PI*(2.*m-1.)*(2.*n-1.)*(2.*l-1.)))*std::sin((2.*m-1.)*PI/2.)*std::sin((2.*n-1.)*PI/2.)*std::sin((2.*l-1.)*PI/2.);
//       temp = ((2.*m-1.)*PI/2.)*((2.*m-1.)*PI/2.) + ((2.*n-1.)*PI/2.)*((2.*n-1.)*PI/2.) + ((2.*l-1.)*PI/2.)*((2.*l-1.)*PI/2.);
//       s += a*(std::exp(-temp*t))*std::cos((2.*m-1.)*PI*x/2.)*std::cos((2.*n-1.)*PI*y/2.)*std::cos((2.*l-1.)*PI*z/2.) ;
//      }
// 
//   values[0] = s;
//   values[1] = 0;
