...
Saddle_point_preconditioner::Saddle_point_preconditioner(
    ...
    const BlockVector&)
  : spp_type(t),
    ps_db(Solver<>::default_solver_database()),
    vs_db(Solver<>::default_solver_database()),
    M(&m), velocity_block(), pressure_block(), pressure_mass(),
    ...
    pressure_solver(nullptr), bdryCorrectionMatrix_(),
    poissonMatrixBdry_(nullptr), poissonSolverBdry_(nullptr)
{
  ...
  lsc_update_pressure = db.try_get_value("lsc_update_pressure", false);
  lsc_distinct_b_blocks = db.try_get_value("lsc_distinct_b_blocks", false);
  lsc_stabilized = db.try_get_value("lsc_stabilized", false);
  lsc_alpha = db.try_get_value("lsc_alpha", 0.0);
  lsc_gamma = db.try_get_value("lsc_gamma", 0.0);
  ...
  if (spp_type == Saddle_point_preconditioner::type::lsc
     || spp_type == Saddle_point_preconditioner::type::bd_lsc)
  {
    if (lsc_stabilized)
    {
      pressure_block = M->get_sub_blockfematrix(n_rows - 1, n_rows - 1);
    }
  }
  ...
  }
  {
    ...
    if(vs_db["solver_type"].is("direct"))
    {
      ...
      if (vs_db.try_get_value("mumps_distributed", false))
      {
        velocity_mumps_wrapper->enable_distributed_solution(
          vs_db["max_n_iterations"], vs_db["residual_tolerance"]);
      }
    }
...
Saddle_point_preconditioner::Saddle_point_preconditioner(
    const CompositeOperator<BlockVector>&, type, const ParameterDatabase&)
  : ps_db(Solver<>::default_solver_database()),
    vs_db(Solver<>::default_solver_database())
{
  ErrThrow("Cannot use Saddle_point_preconditioner with a composite operator!");
}

void Saddle_point_preconditioner::setup_pressure_solver()
{
  ...
  if (lsc_stabilized)
  {
    compute_alpha_diagonal(alpha_diagonal);
  }
  ...
  if (ps_db["solver_type"].is("direct"))
  {
    ...
    if (ps_db.try_get_value("mumps_distributed", false))
    {
      Poisson_solver->enable_distributed_solution(
        ps_db["max_n_iterations"], ps_db["residual_tolerance"]);
    }
  }
  else
  {
...
void Saddle_point_preconditioner::update()
{
  ...
  else if (spp_type == Saddle_point_preconditioner::type::lsc
          || spp_type == Saddle_point_preconditioner::type::bd_lsc)
  {
    if (lsc_update_pressure)
    {
      Output::root_info("LSC", "Updating pressure coupling...");

      gradient_block = M->get_combined_submatrix(
          {0, n_cols - 1}, {n_rows - 2, n_cols - 1});

      divergence_block = M->get_combined_submatrix(
          {n_rows - 1, 0}, {n_rows - 1, n_cols - 2});
...
void Saddle_point_preconditioner::solve_pressure(const BlockVector& rhs,
  BlockVector& sol, bool boundary) const
{
  if (boundary)
  {
    this->poissonSolverBdry_->solve(rhs, sol);
  }
  else
  {
#ifdef _MPI
    if (this->Poisson_solver)
    {
      this->Poisson_solver->solve(rhs, sol);
    }
    else
#endif // MPI
    {
      this->pressure_solver->solve(rhs, sol);
    }
  }
}
                                        ...
                                        BlockVector& r) const
{
  ...
  switch (this->spp_type)
  {
    ...
    {
      // non-stabilized case (C = 0):
      //
      // solve
      //
      // p = -S_p^-1 (B D_v^-1 A D_v^-1 B') S_p^-1 z_p,
      //
      // where S_p = B D_v^-1 B', then solve
      //
      // u = A^-1 (z_u - B' p).
      ...
      // solve
      //
      // p = (-S_p^-1 (B D_v^-1 A D_v^-1 B') S_p^-1 - \alpha D^-1) z_p,
      //
      // where
      // S_p = B D_v^-1 B' - \gamma D_r^-1/2 C D_r^-1/2,
      // D_r = \nu |diag(B D_v^-1 B') / diag(C)| componentwise,
      // D = Diag(B diag(A)^-1 B' - C),
      // \alpha = 1 / \rho(B diag(A)^-1 B' D^-1),
      // \gamma = \rho(D_v^-1 A) / (3 |D_r|_\infty)
      // then solve
      //
      // u = A^-1 (z_u - B' p).
      ...
      solve_pressure(p_tmp, p_star, correct_boundary);
      ...
      u_comm.queue_consistency_update(u_tmp.block(0), 3);
      u_comm.queue_consistency_update(u_tmp.block(1), 3);
      u_comm.queue_consistency_update(u_tmp.block(2), 3);
      ...
      u_comm.queue_consistency_update(u_star.block(0), 3);
      u_comm.queue_consistency_update(u_star.block(1), 3);
      u_comm.queue_consistency_update(u_star.block(2), 3);
      TParFECommunicator3D::flush_consistency_updates();
#endif
      ...
      if (lsc_stabilized)
      {
        const double* z_p = z.block(n_blocks - 1);

        for (unsigned int i = 0; i < alpha_diagonal.size(); i++)
        {
          p_star[i] += alpha_diagonal[i] * z_p[i];
        }
      }
      ...
      Output::root_info("LSC", "Schur complement approximation: ", lsc_timer.time_since_last_start(), " s");
      lsc_timer.stop();
      lsc_timer.start();
      ...
      // MPI: No update of u_tmp necessary, solution 'r'
      ...
      Output::root_info("LSC", "Velocity solver and damping: ", lsc_timer.time_since_last_start(), " s");
  ...
  bool additive_storage) const
{
  ...
  switch(this->spp_type)
  {
    ...
    case Saddle_point_preconditioner::type::bd_lsc: // bdry corrected LSC
    {
      ...
      if (lsc_distinct_b_blocks)
      {
#ifndef _MPI
        ret.reset(divergence_block->multiply_with_pseudotranspose_from_right(
          *gradient_block, inverse_diagonal));
      ...
      if (lsc_stabilized)
      {
        ...
        // copy!
        TMatrix block_C(*pressure_block.get_block(0, 0, transpose));

        if (transpose)
        {
          ErrThrow("Can't handle a transposed C block!");
        }

        auto diag_Poisson = ret->get_diagonal();
        auto diag_C = block_C.get_diagonal();
        unsigned int n = diag_C.size();

        double ratio_max = 0.0;

#ifdef _MPI
        const TParFECommunicator3D* p_comm = M->get_communicators()[3];
        const int* p_master = p_comm->GetMaster();
        int rank;
        MPI_Comm_rank(MPI_COMM_WORLD, &rank);
        p_comm->queue_consistency_update(diag_Poisson.data(), 3);
        p_comm->queue_consistency_update(diag_C.data(), 3);
        TParFECommunicator3D::flush_consistency_updates();
#endif

        unsigned int C_zeros = 0;

        // diag_C <- diag(D_r^1/2)
        // where D_r_ii = |(B D_v^-1 B')_ii / C_ii|
        // note that since we're multiplying C by D_r^1/2 from either side and
        // \gamma by 1 / |D_r|_\infty, the scaling of C does not matter here
        for (unsigned int i = 0; i < n; i++)
        {
          double v = diag_Poisson[i];
          double w = diag_C[i];
          ...
          diag_C[i] = std::sqrt(ratio);
        }

#ifdef _MPI
        MPI_Allreduce(MPI_IN_PLACE, &ratio_max, 1, MPI_DOUBLE,
          MPI_MAX, MPI_COMM_WORLD);
        MPI_Allreduce(MPI_IN_PLACE, &C_zeros, 1, MPI_UNSIGNED,
          MPI_SUM, MPI_COMM_WORLD);

        p_comm->consistency_update(diag_C.data(), 3);
#endif

        if (C_zeros > 0)
        {
          Output::root_warn("LSC", "C has ", C_zeros, " zeros on the diagonal");
        }

        // spectral radius of D_v^-1 A
        double rho = velocity_block.estimate_spectral_radius(
          inverse_diagonal.data());

        double gamma = ratio_max > 0.0 ? (lsc_gamma * rho / (3.0 * ratio_max)) : 0.0;

        Output::root_info("LSC", "\\rho(D_v^-1 A) ~= ", rho, ", "
          "|D_r|_\\infty = ", ratio_max,
          " => \\gamma = ", lsc_gamma, " * \\rho / (3 |D_r|_\\infty) = ", gamma);

        // block_C <- D_r^1/2 C D_r^1/2
        block_C.scale(diag_C.data(), true);
        block_C.scale(diag_C.data(), false);

        // ret <- B D_v^-1 B' - \gamma D_r^1/2 C D_r^1/2
#ifdef _MPI
        ret->add_scaled(block_C, -gamma,
          *M->get_communicators()[3], additive_storage);
#else
        ret->add_scaled(block_C, -gamma);
#endif
      }
  ...
  if (additive_storage)
  {
    auto ptr = std::make_shared<BlockFEMatrix>(1, 1, vec);

    ptr->set_additive(true);
...
/* LSC === */
void
Saddle_point_preconditioner::compute_alpha_diagonal(
  std::vector<double> &diag) const
{
  std::shared_ptr<TMatrix> temp_mat(nullptr);

  if (spp_type != Saddle_point_preconditioner::type::lsc)
  {
    ErrThrow("compute_alpha_diagonal is only supported with LSC");
  }

  size_t n_u_diagonal_entries = velocity_block.get_n_total_rows();
  size_t n_p_diagonal_entries = pressure_block.get_n_total_rows();
  diag.resize(n_u_diagonal_entries);

  // diag <- diag(A)^-1
  for (size_t i = 0; i < n_u_diagonal_entries; i++)
  {
    double v = velocity_block.get(i, i);

    if (v == 0.0)
    {
      Output::warn("LSC", "A has a zero on the diagonal at row ", i);
      diag[i] = 0.0;
    }
    else
    {
  ...
  // temp_mat <- B diag(A)^-1 B'
#ifdef _MPI
  const TParFECommunicator3D* u_comm = M->get_communicators()[0];
  const TParFECommunicator3D* p_comm = M->get_communicators()[3];

  int n = u_comm->GetNDof();

  u_comm->queue_consistency_update(diag.data(), 3);
  u_comm->queue_consistency_update(diag.data() + n, 3);
  u_comm->queue_consistency_update(diag.data() + 2 * n, 3);
  TParFECommunicator3D::flush_consistency_updates();

  if (lsc_distinct_b_blocks)
  {
    temp_mat.reset(divergence_block->multiply_with_pseudotranspose_from_right(
      *gradient_block,
      diag, { p_comm }, { u_comm, u_comm, u_comm }, true));
  }
  else
  {
    temp_mat.reset(divergence_block->multiply_with_transpose_from_right(
      diag, { p_comm }, { u_comm, u_comm, u_comm }, true));
  }
#else
  if (lsc_distinct_b_blocks)
  ...
  bool transpose;
  auto block_C = pressure_block.get_block(0, 0, transpose);

  diag.resize(n_p_diagonal_entries);

  // diag <- diag(B diag(A)^-1 B')
  for (size_t i = 0; i < n_p_diagonal_entries; i++)
  {
    diag[i] = temp_mat->get(i, i);
  }

#ifdef _MPI
  p_comm->consistency_update(diag.data(), 3);
#endif

  // diag <- diag(D)^-1 = diag(B diag(A)^-1 B' - C)^-1
  for (size_t i = 0; i < n_p_diagonal_entries; i++)
  {
    double v = diag[i] - block_C->get(i, i);
    ...
    {
      Output::warn("LSC", "B diag(A)^-1 B^T - C has a zero on the diagonal at "
        "row ", i);
      diag[i] = 0.0;
    }
  }
  ...
  // temp_mat <- B diag(A)^-1 B' D^-1
  // (scale columns)
  temp_mat->scale(diag.data(), false);

  auto fe_matrix = std::make_shared<FEMatrix>(pressure_space, *temp_mat);
  std::vector<std::shared_ptr<FEMatrix>> vec(1, fe_matrix);

  auto block_fe_matrix = std::make_shared<BlockFEMatrix>(1, 1, vec);

#ifdef _MPI
  block_fe_matrix->set_additive(true);
#endif

  double rho = block_fe_matrix->estimate_spectral_radius();
  double alpha = lsc_alpha;

  if (rho != 0.0)
  {
    alpha /= rho;

    Output::root_info("LSC", "\\rho(B diag(A)^-1 B^T D^-1) ~= ", rho,
      " => \\alpha = ", lsc_alpha, " / \\rho = ", alpha);
  }
  else
  {
    Output::root_warn("LSC", "B diag(A)^-1 B^T D^-1 has zero spectral radius");
    alpha = 0.0;
  }
...
void Saddle_point_preconditioner::fill_pressure_mass_matrix()
{
     ...
     || (this->spp_type == Saddle_point_preconditioner::type::mod_AL))
  {
    ...
    Assemble2D(n_fe_spaces, &q_space, n_matrices, sq_matrices, n_rect_mat,
               ...
               non_const_bound_values, la, true);
