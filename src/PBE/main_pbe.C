...
int main()
{
	...
	double Lmin = L_min, Lmax = Lmin * std::pow(nxInit * std::pow(2, L - 1) + 1, 1.0/3.0);
	double l1 = 0.01, l2 = 0.01, l3 = 0.5;

	// TO BE CHANGED WITH REAL VELOCITY / SHEAR FLOW
	double* v = allocate_vector(nr1 * nr2 * nr3 * 3);
	fill_vector(v, nr1 * nr2 * nr3 * 3, std::abs((double)rand() / (double)20000));
	double* grad_v = allocate_vector(nr1 * nr2 * nr3 * 9);
	fill_vector(grad_v, nr1 * nr2 * nr3 * 9, std::abs((double)rand() / (double)10000));
