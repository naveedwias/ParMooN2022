...
double Breakage::get_value_at_x(double* f, double x0) {
	int i = get_uniform_index(std::pow(xmax, 3) / na, std::pow(x0, 3));
...
double Breakage::definite_integral(double& fa, double& fb, double& a, double& b, double c, double& x1, double& x2) {
	...
	return alpha * d4 / 5 + (alpha * c - beta) * 
		   (d4 / 4 + c * d3 / 3 + ipow(c, 2) * d2 / 2 + ipow(c, 3) * (x2 - x1) + ipow(c, 4) * (std::log(std::abs(x2 - c)) - std::log(std::abs(x1 - c))));
}
...
void Breakage::breakage_plus(double* input, double* output, double* v) {
	int i, j; 
	double b = 0;
    double E = std::pow(0.5 * ro_d * k_V * ipow(euclidean_norm(v, 3), 2), 1.0/3.0);
	double CE = C * E;

	if(CE < ZERO_THRESHOLD)
		return;

	double factor = std::pow(0.5, 0.25) * ipow(CE, 3) / k_V;
	double y_c = std::pow(2 * L_min, 0.75) / CE;
	...
	for(i = na - 1; i >= 0; i--) {
		...
		output[i] += (int1D + si) * factor * std::pow(a[i], a[i] == 0 ? 1 : -3.25);
	}
	...
	for(i = 0; i < na; i++) {
		...
		for(int j = 0; j < croots; j++)
			if(yj > g[i] && yj < xmax) {
				output[i] += get_value_at_x(input, yj) * 3 * std::pow(sqr(C_r * yj - 1),	1.0/3.0)
							 / (4 * C_r * yj - 3);
			}
	}
