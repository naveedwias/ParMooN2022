...
koeffizienten_xi_1dim(int grad) {
	...
	for (n=0;n<=2*grad;n++) {
		a[n] = std::sqrt(2.*n+3)*std::sqrt(2.*n+1)/(fepc_real_t)(n+1);
	}
	b[0] = 0.0;
	for (n=1;n<=2*grad;n++) {
		b[n] = (std::sqrt(2.*n+3)/std::sqrt(2.*n-1)) * ( (fepc_real_t)n/(fepc_real_t)(n+1) );
	}
	...
	for (q=1;q<=2*grad;q++) {
		for (n=0;n<=q;n++) {
			...
			if (n<m) {
				...
				faktor = std::pow(2.,((fepc_real_t)n+0.5))/std::sqrt( (2.*n+1)*(2*m+1) );
				reell = temp;
				rat = reell*faktor;
...
koeffizienten_gamma_1dim(int grad) {
	...
	for (k=1;k<=3*grad;k++) {
  	temp = 2.0*std::sqrt( (2.0*k-1)*(2.0*k+1) );
		g[0][k-1][k] = std::pow(-1.,k)/temp;
	}
	for (k=1;k<=3*grad;k++) {
		g[0][k][k-1] = g[0][k-1][k];
	}
	...
	for(q=1;q<=3*grad;q++) {
		for(a=1;a<=q;a++) {
			for(b=0;b<=(q-a);b++) {
				...
				if (b==0) {
					g[a][0][k] = std::pow(-1.,a) * g[0][a][k];
				}
				if (k==0) {
					g[a][b][0] = std::pow(-1.,a) * g[0][b][a];
				}
				if ( ((a>b+k+1)||(b>a+k+1)||(k>a+b+1)||(b==0)||(k==0)) == 0) {
					...
					g[a][b][k] = temp;
					/*Korrektur der Werte (siehe Dokumentation)*/
					wurzel = std::sqrt( (2.*a+1)*(2*b+1)*(2*k+1) );
					reell = temp;
					rat = reell*wurzel;
					rat = get_rational(rat,1e-14,10000);
					reell = rat/wurzel;
...
koeffizienten_gamma(int level, int r, fepc_real_t mesh, int alpha, int mu, int kappa, matrix3_p gamma_koef) {
	...
	temp = std::sqrt( mesh ) / std::pow(2.0, (fepc_real_t)level / 2.0 );
