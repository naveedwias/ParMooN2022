...
int solveQuadraticEquation(idouble*  res, double p_dA, double p_dB, double p_dC)
{
	...
	if (delta >= 0) { // Two real solutions
		res[0].m_dReal = (-a1 + std::sqrt(delta)) / 2;
		res[1].m_dReal = (-a1 - std::sqrt(delta)) / 2;
...
int solveQuadraticEquationReal(double*  res, int& cRealRoots, double p_dA, double p_dB, double p_dC)
{
	...
	cRealRoots = 0;
	if (delta >= 0) {
		res[0] = (-a1 + std::sqrt(delta)) / 2;
		res[1] = (-a1 - std::sqrt(delta)) / 2;
...
int solveQuadraticEquationReal1(double*  res, int& cRealRoots, double p_dB, double p_dC)
{
	...
	if (delta >= 0) {
		res[0] = 0.5 * (-p_dB + std::sqrt(delta));
		res[1] = 0.5 * (-p_dB - std::sqrt(delta)) ;
...
int solveCubicEquation(idouble* res, double p_dA, double p_dB, double p_dC, double p_dD)
{
	...
	double theta = std::atan2(wr.m_dImm, wr.m_dReal);
	
	idouble zres[3];
	
	double dCR = cubert(r);
	
	zres[0].m_dReal = dCR * std::cos(theta/3);
	zres[0].m_dImm = dCR * std::sin(theta/3);
	
	zres[1].m_dReal = dCR * std::cos((theta + TWOPI)/3);
	zres[1].m_dImm = dCR * std::sin((theta + TWOPI)/3);
	
	zres[2].m_dReal = dCR * std::cos((theta + FOURPI)/3);
	zres[2].m_dImm = dCR * std::sin((theta + FOURPI)/3);
...
int solveQuarticEquationReal(double* res, int& cRealRoots, double p_dA, double p_dB, double p_dC, double p_dD, double p_dE)
{
	...
	if (f == 0 && g == 0 && e == 0) {
		...
		if(croots > 0) {
			if(qres[0] >= 0) {
				double z1 = std::sqrt(qres[0]);
				res[cRealRoots++] = z1 - quarter_of_a;
				res[cRealRoots++] = -z1 - quarter_of_a;
			}
			if(qres[1] >= 0) {
				double z2 = std::sqrt(qres[1]);
