...
void Aggregation::fill_l2m_and_m2l_values()
{
	...
	for(int i = 0; i < na; i++) {
		h = a[i+1] - a[i];
		l2m_start_value_k[i] = (std::pow(a[i+1],2./3.) - std::pow(a[i], 2./3.)) * 1.5 / h - 6 / cube(h) * h * 
							( (std::pow(a[i+1], 5./3.) - std::pow(a[i], 5./3.)) * 0.6 - 
								  0.75 * (a[i+1] + a[i]) * ( std::pow(a[i+1], 2./3.) - std::pow(a[i], 2./3.) ) );
		l2m_end_value_k[i] = (std::pow(a[i+1], 2./3.) - std::pow(a[i], 2./3.)) * 1.5 / h + 6 / cube(h) * h * 
							( (std::pow(a[i+1], 5./3.) - std::pow(a[i], 5./3.)) * 0.6 - 
							   0.75 * (a[i+1] + a[i]) * ( std::pow(a[i+1], 2./3.) - std::pow(a[i], 2./3.) ) );
		l2m_start_value_b[i] = (std::pow(a[i+1], 1./3.) - std::pow(a[i], 1./3.)) * 3 / h - 6 / cube(h) * h * 
							( (std::pow(a[i+1], 4./3.) - std::pow(a[i], 4./3.)) * 0.75 - 
							   1.5 * (a[i+1] + a[i]) * ( std::pow(a[i+1], 1./3.) - std::pow(a[i], 1./3.) ) );
		l2m_end_value_b[i] = (std::pow(a[i+1], 1./3.) - std::pow(a[i], 1./3.)) * 3 / h + 6 / cube(h) * h * 
							( (std::pow(a[i+1], 4./3.) - std::pow(a[i], 4./3.)) * 0.75 - 
							   1.5 * (a[i+1] + a[i]) * ( std::pow(a[i+1], 1./3.) - std::pow(a[i], 1./3.) ) );

		h = cubert(a[i+1]) - cubert(a[i]);
		m2l_start_value_k[i] = (sqr(a[i+1]) - sqr(a[i])) / (6 * h) - 6 / cube(h) * h * 
							( (std::pow(a[i+1], 7./3.) - std::pow(a[i], 7./3.)) / 7. - 
							  (cubert(a[i+1]) + cubert(a[i])) * ( sqr(a[i+1]) - sqr(a[i])) / 12. );
		m2l_end_value_k[i] = (std::pow(a[i+1], 2.) - std::pow(a[i], 2.)) / (6 * h) + 6 / cube(h) * h * 
							( (std::pow(a[i+1], 7./3.) - std::pow(a[i], 7./3.)) * 1./7. - 
							  (cubert(a[i+1]) + cubert(a[i])) * ( std::pow(a[i+1], 2.) - std::pow(a[i], 2.)) / 12. );
		m2l_start_value_b[i] = (std::pow(a[i+1], 1.) - std::pow(a[i], 1.)) / (3 * h) - 6 / cube(h) * h * 
							( (std::pow(a[i+1], 4./3.) - std::pow(a[i], 4./3.)) * 1./4. - 
							  (cubert(a[i+1]) + cubert(a[i])) * (a[i+1] - a[i]) / 6. );
		m2l_end_value_b[i] = (std::pow(a[i+1], 1.) - std::pow(a[i], 1.)) / (3 * h) + 6 / cube(h) * h * 
							( (std::pow(a[i+1], 4./3.) - std::pow(a[i], 4./3.)) * 1./4. - 
							  (cubert(a[i+1]) + cubert(a[i])) * (a[i+1] - a[i]) / 6. );
	}
}
...
//    if (std::abs((wb0.coeff[j] + wb1.coeff[j]))>1.e-3)
//    {
...
// 	Output::print("Total mass of the psd was ", input_mass);
//  Output::print("Total mass of the right-hand side was ", rhs_mass);
...
int aggregation::create_hierarchical_structure()
{
	...
	L = iround(std::log(h_max / h_min, 2.0)) + 1;
	length = new double[L];
	hl = new double[L];
	...
	for(int l = 0; l < L; l++)
	{
		...
		hl[l] = hl[L-1] * std::pow(2., L-l-1);
		sqrt_hl[l] = std::sqrt(hl[l]);
	}
...
void aggregation::convert_func_p0_to_piecewise_constant(func_p0_p& function, piecewise_constant& pl)
{
	for(int l = 0; l < L; l++)
	{
		double hl = stepping * std::pow(0.5, l);
		for(int i = count[l] - count2[l]; i < count[l]; i++)
		{
			pl.value[nl[l] + i - (count[l] - count2[l])] = function->hierarchie[l]->vektor->glied[i] * std::sqrt(1./hl);
		}
	}
}
