...
double euclidean_norm(double* a, int n) {
	return std::sqrt(scalar_product(a, a, n));
}
...
double min_abs_vector(double* v, int n) {
	int i;
	double ret = std::abs(v[0]);

	for(i = 1; i < n; i++)
		if(ret > std::abs(v[i])) ret = std::abs(v[i]);
...
double std::log(double x, double a) {
	if(a <= 0) return -1;
...
double cubert(double x) { // Gives the cube root number ( cubert(-8) = -2)
	return std::pow(std::abs(x), 1.0/3.0) * sgn(x);
}
...
void fill_hrefined_grid(double a, double b, int cLevels, int n, double* grid) {
	int c = 1, l, i;
	double h = (b - a) / (double)(n * std::pow(2.0, cLevels - 1));
	grid[0] = a;
...
void fill_exponential_data(function_1D f, int cLevels, int n, double* d) {
	int c = 0, l, i;
	double x = 0, h = 1.0 / (double)(n * std::pow(2.0, cLevels - 1));
...
void transform_grid_m2l(double& xmin, double& xmax, int n, double* x) {
	xmin = std::pow(xmin, 1.0 / 3.0);
	xmax = std::pow(xmax, 1.0 / 3.0);
	for(int i = 0; i < n; i++)
		x[i] = std::pow(x[i], 1.0/3.0);
}
...
int get_index(double* x, int n, double x0) {
	// TODO: via bisection, in O(std::log(n)) flops
	int i = 0;
	return i;
}
