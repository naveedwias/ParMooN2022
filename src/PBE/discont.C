...
integrate_coeff_discont(discont_function_p function, int v, int position, int p, int level, fepc_real_t stepping) {
    ...
    if (p == 0) { // legendre(0) = std::sqrt(1/h_l)
        return std::sqrt(h_l)*(y_0 + h_l*slope*(v+0.5));
    } else { // p == 1 --> legendre(1) = std::sqrt(12)(x-(v+0.5)*h_l)/(h_l^1.5)
        return h_l*std::sqrt(h_l)*slope/SQRT_12;
    }
}
