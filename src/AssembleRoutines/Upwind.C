...
void GetUpwindValue(double UPWIND_ORDER,double t,double *phi)
{
  ...
  else
  {
    ...
    {
      s = std::abs(t);
      *phi = 0.5+0.5*sgn(t)*std::pow(s/(s+1), UPWIND_ORDER);
    }
  }
  return;
}
...
//  };
  ...
  coeffs = new double[6];
  params = new double[9];
  memset(params, 0, 9 * sizeof(double));

  // get pointers to columns, rows and entries of matrix A
  ColInd = sqmatrix->get_vector_columns();
  RowPtr = sqmatrix->get_row_ptr();
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    switch(CurrentElement)
    {
        ...
        for(k=0;k<N_Vertices;k++)
        {
          ...
          switch(UPW_APPL)
          {
            ...
            case 1:
              ...
              break;
            default:
              Output::print("Wrong Upwind Application !!! Default is used !!!");
              matrix[k][k] += flux*phi;
              matrix[k][j] -= flux*phi;
              matrix[j][k] += flux*(1-phi);
              matrix[j][j] -= flux*(1-phi);
      ...
      default:
        ErrThrow("Upwind works only for first order nonconforming elements!");
    }                                             // endswitch
                       ...
                       bool ConvIsVelo)
{
  ...
  memset(param, 0, 9 * sizeof(double));
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    switch(CurrentElement)
    {
      ...
      default:
        ErrThrow("Upwind works only for first order elements!");
        break;
    }                                             // endswitch
    for(m=0;m<N_Vertices;m++)
    {
...
//  TBoundComp *BoundComp;
  ...
  if (N_Edges != 3)
  {
    ErrThrow("Improved Mizukami-Hughes Method implemented only for triangles !!!");
  }
  sx = sy = 0;
  ...
  for (i=0;i<N_Edges;i++)
  {
    ...
    if ((val[i] > 0) && (val[(i+1)%N_Edges] <= 0) && (val[(i+2)%N_Edges] <= 0))
    {
      if (found > -1)
      {
        ErrThrow(cell_no, " already something found ", found,
                 " found new in vertex zone ", i);
      }
      found = 2*i;
    }
    // edge zone
  ...
  if (found==-1)
  {
    ErrThrow("Mizukami-Hughes: nothing found !!!");
  }
...
//      hy = y[1] - y[0];
  ...
  if (std::abs(deter)<1e-20)
  {
    // parallel lines, no intersection
    //Output::print("Determinant vanishes !!!");
    alpha = -4711;
  }
  else
  {
  ...
  // Output::print(b1+alpha_a*wx, " ", sx+beta*h31x, "   ",
  //	   b2+alpha_a*wy, " ", sy+beta*h31y, "   a ", alpha, " b ", beta);
  if ((alpha<=0)&&(beta<=0))
  {
    ErrThrow("negative ", found, " S ", sx, " ", sy, " h21 ", h21x, " ", h21y,
             " h31 ", h31x, " ", h31y, " w ", wx, " ", wy, " b ", b1, " ", b2,
             " a ", alpha, " ", beta);
  }
  ...
  s1 = b1/norm_b;
  s2 = b2/norm_b;
  h21 = std::sqrt(h21x*h21x+h21y*h21y);
  h21x /= h21;
  h21y /= h21;
  h31 = std::sqrt(h31x*h31x+h31y*h31y);
  h31x /= h31;
  h31y /= h31;
  vx = h21x + h31x;
  vy = h21y + h31y;
  ...
  if (wx*vx_orth + wy*vy_orth < 0)
  {
    ...
    t0 = std::abs(s1*h21y-s2*h21x);
    t1 = std::abs(vx*h21y-vy*h21x);
    r2 = t0/t1 + 1 - r2;
    if (r2>1)
      r2 = 1;
    t0 = std::abs(wx*h21y - wy*h21x);
    t1 = vx*h21x + vy*h21y;
    t1 *= r2;
    phi = 2*t0/t1;
    if (phi>1)
    ...
    // second case
  {
    ...
    t0 = std::abs(s1*h31y-s2*h31x);
    t1 = std::abs(vx*h31y-vy*h31x);
    r2 = t0/t1 + 1 - r2;
    if (r2>1)
      r2 = 1;
    t0 = std::abs(wx*h31y - wy*h31x);
    t1 = vx*h31x + vy*h31y;
    t1 *= r2;
    phi = 2*t0/t1;
    if (phi>1)
...
//  int *N_BaseFunct;// LocN_BF;
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    {
      ErrThrow("Mizukami-Hughes method implemented only for P_1 and not for fe id ", CurrentElement);
    }
