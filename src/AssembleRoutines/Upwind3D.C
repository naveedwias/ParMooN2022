...
//  int n_mat, m_mat;
  const double *u1vect, *u2vect, *u3vect;
  double matrix[8][8];
...
/*  double FctValNTetra[6][6] = { {  0.666666666666666666667, 
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    // reset local matrix
    memset(matrix, 0, sizeof(double)*64);
	...
	desc->GetEdgeVertex(TmpEV);
	  ...
	  Area=std::sqrt(nx*nx+ny*ny+nz*nz);
     ...
     // ********** conforming discretization *******************	
      default:
        ErrThrow("Upwind works only for nonconforming elements!");
        break;
    } // endswitch
        ...
        for(n=RowPtr[l];n<end;n++)
        {
          for(k=0;k<N_Faces;k++)
          {
            if(DOF[k] == ColInd[n])
            {
              ...
              //Output::print("E ", Entries[n], " ", matrix[m][k]);
  ...
  for(i=0;i<N_Cells;i++)
  {
	...
	desc->GetEdgeVertex(TmpEV);
     ...
     // ********** conforming discretization *******************
      ...
      default:
        ErrThrow("Upwind is not implemented for conforming discretization !",
                 " Upwind works only for first order elements!");
        break;
    } // endswitch
