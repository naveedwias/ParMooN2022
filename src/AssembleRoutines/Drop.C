...
// are dimentionless on D^1/2  
  ...
  // for the case of magnetic saturation 
    /*
    memset(f1,0,(2*N-1)*sizeof(double));
    f2[0] = 1.; f2[2*N-2] = 0.;
    for(i=1;i<2*N-2;i++)
      if(i%2==0)
	if(adap)
	  f2[i] = std::pow((r[i/2+1]-r[i/2-1])/(s[i/2+1]-s[i/2-1]),2);
	else
	  f2[i] = std::pow((r[i/2+1]-r[i/2-1])/(2.*h),2);
      else
	if(adap)
	  f2[i] = std::pow((r[i/2+1]-r[i/2])/(s[i/2+1]-s[i/2]),2);
	else
	  f2[i] = std::pow((r[i/2+1]-r[i/2])/h,2);
    */
  // taking into considaration the magnetic field
    ...
    for(i=0;i<N;i++){
      ...
      h_r[i] = (60 - 40*r[i]/r[N-1])*std::cos(M_PI/180.*(90+15));
      h_z[i] = (60 - 40*r[i]/r[N-1])*std::sin(M_PI/180.*(90+15));
    }
	...
	H = std::abs(h_z[N-1]);
	  ...
	  H = std::sqrt(h_r[i/2]*h_r[i/2] + h_z[i/2]*h_z[i/2]);
        }
        else
        {
	  if(adap)
	  ...
	  H = std::sqrt(std::pow((h_r[i/2+1]+h_r[i/2])/2.,2) + 
		   std::pow((h_z[i/2+1]+h_z[i/2])/2.,2));
        }
      // for linear case
      f1[i] = H*H;
      f2[i] = Hn*Hn;
      // for nonlinear case      
      // arg = H*gamma;
      // f1[i] = std::log(std::sinh(arg)/arg);    
      // f2[i] = std::pow((1./std::tanh(arg)-1./arg)*Hn/H,2);    
    }          
    //Output::print("RHS ");
    for(i=0;i<2*N-1;i++)
      {
...
//  ----------------------------------------------  
    ...
    if(adap)
      {
	for(i=1;i<N;i++)
	  C+=std::pow((r[i-1]+r[i])/2.,K)*(r[i]-r[i-1])*f[2*i-1];
	C = -std::pow(2.,K)/r[N-1]*(1. + C/std::pow(r[N-1],K));
      }
    else
      {
	for(i=1;i<N;i++)
	  C+=std::pow((r[i-1]+r[i])/2.,K)*(r[i]-r[i-1])/h*f[2*i-1];
	C = -std::pow(2.,K)/r[N-1]*(1. + h*C/std::pow(r[N-1],K));
      }
    // for reometer problem
    C = 0.;
    for(i=1;i<N;i++)
	  C+=(r[i]-r[i-1])*f[2*i-1];
    C = L*(-C+2*std::cos(alpha));
    Output::print("C=", C);
    ...
    {
      if(adap)
	err = std::abs((r[i-1]/dsm[i]-r[i]*(1./dsm[i]+1./dsm[i+1])+r[i+1]/dsm[i+1])/h/h+
		  (z[i+1]-z[i-1])/(2.*h)*
		  (f[2*i] - K*(z[i+1]-z[i-1])/(2.*h)/r[i]/ds[i] + C));
      else
	err = std::abs((r[i-1]-2.*r[i]+r[i+1])/h/h+
		  (z[i+1]-z[i-1])/(2.*h)*
...
//        i_err_r = i;
    ...
    {
      if(adap)
	err = std::abs((z[i-1]/dsm[i]-z[i]*(1./dsm[i]+1./dsm[i+1])+z[i+1]/dsm[i+1])/h/h-
		  (r[i+1]-r[i-1])/(2.*h)*
		  (f[2*i] - K*(z[i+1]-z[i-1])/(2.*h)/r[i]/ds[i] + C));
      else
	err = std::abs((z[i-1]-2.*z[i]+z[i+1])/h/h-
		  (r[i+1]-r[i-1])/(2.*h)*
		  (f[2*i] - K*(z[i+1]-z[i-1])/(2.*h)/r[i] + C));
      if(err>err_z)
      { 
...
// b
    if(adap)
      b[0] = h*h/2.*(f[0] - K*(f[0]+C)/2. + C)*std::pow(ds[0],2);
    else
      b[0] = h*h/2.*(f[0] - K*(f[0]+C)/2. + C);
    // for reometer problem
    b[0] = -h*std::cos(alpha)+h*h/2.*std::sin(alpha)*(f[0]+C);
...
// b
    ...
    if(adap)    
      b[N-1] = -h*h/2.*(f[2*N-2] + K/r[N-1] + C)*std::pow(ds[N-1],2);
    else
      b[N-1] = -h*h/2.*(f[2*N-2] + K/r[N-1] + C);
    // for reometer problem
    b[N-1] = h*std::sin(alpha)+h*h/2.*std::cos(alpha)*(f[2*N-2]+C);
    
    Solver_3diag(N,c,d,e,b);
    memcpy(r,b,N*sizeof(double));
    memcpy(z,temp,N*sizeof(double));
    
    if(it>10000) {ID = 0; Output::print("Calculation was stoped"); break;}
    
    //Output::print("Surface");
    //for(i=0;i<N;i++)
    //  Output::print(r[i], ' ', z[i]);
    Output::print("It = ", it, ", err_r = ", err_r, ", err_z = ", err_z);
  }
  Output::print("It = ", it, ", err_r = ", err_r, ", err_z = ", err_z);
  ...
  *LL = L;
  if (adap) 
    {    
      *a = A(-(f[0]+C)/(L),h);
      Output::print("curv = ", -(f[0]+C)/L, " a = ", *a, " h(0) = ",
                    S(*a,t[1])-S(*a,t[0]));
    }
...
// are dimentionless on V^1/3
  ...
  while(it<5 || err_r>eps || err_z>eps)
  {
    it++;
     ...
     L = 0.;
    for(i=1;i<N;i++)
      L+=std::pow((r[i-1]+r[i])/2.,2)*(z[i]-z[i-1])/h;
    L = std::pow(-2*M_PI*h*L,-1./3.);
  ...
  // taking into considaration the magnetic field
    ...
    for(i=0;i<2*N-1;i++)
    {
	  ...
	  if(adap)
	    ...
	    Hn = -(z[i/2+1]-z[i/2-1])/(2.*h)*h_r[i/2] + 
	      (r[i/2+1]-r[i/2-1])/(2.*h)*h_z[i/2];
	  H = std::sqrt(h_r[i/2]*h_r[i/2] + h_z[i/2]*h_z[i/2]);
	  if(it%1==0) Output::print(z[i/2], ' ', Hn);
        }
        else
        {
	  if(adap)
	  ...
	  H = std::sqrt(std::pow((h_r[i/2+1]+h_r[i/2])/2.,2) + 
		   std::pow((h_z[i/2+1]+h_z[i/2])/2.,2));
	  if(it%1==0) Output::print((z[i/2+1]+z[i/2])/2., ' ', Hn);
        }
      // for linear case
      f1[i] = H*H;
      f2[i] = Hn*Hn;
    ...
    //if(it%1000==0) Output::print("rhs");
    for(i=0;i<2*N-1;i++)
      {
	// for linear case
	f[i] = -W*L*f1[i]-W*L*hi*f2[i]; 
	//if(it%1000==0) Output::print(i, ' ', f[i]);
	// for nonlinear case
	// f[i] = -2.*W*L/3./hi*f1[i]-W*L*f2[i];    
      }
//  ----------------------------------------------  
...
// b
    if(adap)
      b[0] = h/2.*(f[0] - K*(f[0]+C)/2. + C)*std::pow(ds[0],2);
    else
...
// b
    ...
    if(adap)    
      b[N-1] = -h/2.*(f[2*N-2] + K/r[N-1] + C)*std::pow(ds[N-1],2);
    else
    ...
    if(it>100000) {ID = 0; Output::print("Calculation was stoped"); break;}
  }
  Output::print("It = ", it, ", err_r = ", err_r, ", err_z = ", err_z);
  ...
  *LL = L;
  if (adap) 
    {    
      *a = A(-(f[0]+C)/(2.*L),h); 
      Output::print("curv = ", -(f[0]+C)/(2.*L), " a = ", *a, " h(1) = ",
                    S(*a,t[1])-S(*a,t[0]));
    }
...
// are dimentionless on V^1/3
	...
	((hi==21) && (it<1)))
  {
    ...
    {
      Hn = h_n[i];
      H = std::sqrt(h_s[i]*h_s[i] + h_n[i]*h_n[i]);
      // for linear case
      f1[i] = H*H;
      f2[i] = Hn*Hn;
    ...
    //if(W>=5.724 && it==1) Output::print("F ", it);
    for(i=0;i<N+1;i++)
      {
	// for linear case
	f[i] = -W*L*f1[i]-W*L*hi*f2[i]; 
	//if(W>=5.724 && it==1 && i==0) Output::print(z[i], ' ', f[i]+C);
	//if(W>=5.724 && it==1 && i>0 && i!=N) Output::print((z[i-1]+z[i])/2., ' ', f[i]+C);
	//if(W>=5.724 && it==1 && i==N) Output::print(z[N-1], ' ', f[N]+C);
	// for nonlinear case
	// f[i] = -2.*W*L/3./hi*f1[i]-W*L*f2[i];    
      }
//  ----------------------------------------------  
    ...
    if(adap)
      {
	for(i=1;i<N;i++)
	  C+=std::pow((r[i-1]+r[i])/2.,K)*(r[i]-r[i-1])*f[i];
	C = -std::pow(2.,K)/r[N-1]*(1. + C/std::pow(r[N-1],K));
      }
    else
      {
	for(i=1;i<N;i++)
	  C+=std::pow((r[i-1]+r[i])/2.,K)*(r[i]-r[i-1])/h*f[i];
	C = -std::pow(2.,K)/r[N-1]*(1. + h*C/std::pow(r[N-1],K));
      }
    //Output::print("C/L ", C/L);
...
// b
	...
	if(W>=5.724 && it>=15000 && it%10==0 && L>1.4) 
	  {
	    //Output::print(i, ' ', (z[i-1]/dsm[i]-z[i]*(1./dsm[i]+1./dsm[i+1])+z[i+1]/dsm[i+1])*(1.-tau),
	    //   ' ', tau*h*(((r[i]-r[i-1])*f[i]+(r[i+1]-r[i])*f[i+1])/2.), ' ', 
	    //	   tau*h*(r[i+1]-r[i-1])/(2.)*(-K*(z[i+1]-z[i-1])/(2.*h)/r[i]/ds[i] + C));
	    //Output::print(i, ' ', (r[i+1]-r[i-1])/(2.), ' ', (z[i+1]-z[i-1])/(2.*h), ' ',
	    //	   r[i], ' ', ds[i]);
	  }
      }
    b[N-1] = 0.;
...
// b
    ...
    if(adap)    
      b[N-1] = -h*h/2.*(f[N] + K/r[N-1] + C)*std::pow(ds[N-1],2);
    else
    ...
    */
	...
	for(i=0;i<N;i++)
	  {
	    Output::print(r[i]*L, ' ', z[i]*L, ' ', f[i]+C);
	  }
	Output::print("L="<<L<<" C=", C);
      }
    */
    if(it>100000) {ID = 0; Output::print("Calculation was stoped"); break;}
  }
  ...
  L = 0.;
    ...
    //if(W>=5.724) Output::print("Surface");
    for(i=0;i<N;i++)
      {
	r[i]*=L; z[i]*=L;
	//if(W>=5.724) Output::print(r[i], ' ', z[i]);
      }
	...
	for(i=0;i<N+1;i++)
	  {
	    if(i==0) Output::print(z[i], ' ', f[i]+C);
	    if(i>0 && i!=N) Output::print((z[i-1]+z[i])/2., ' ', f[i]+C);
	    if(i==N) Output::print(z[N-1], ' ', f[N]+C);
	  }
	Output::print("C=", C, ' ', "C/L=", C/L);
      }
    */
  *LL = L;
  if (adap) 
    {    
      *a = A(-(f[0]+C)/(2.*L),h); 
      //Output::print("curv = ", -(f[0]+C)/(2.*L), " a = ", *a, " h(1) = ", S(*a,t[1])-S(*a,t[0]));
    }
...
//  ----------------------------------------------  
    ...
    for(i=0;i<N;i++)
    {
      err+=std::pow(std::abs(r[i]-r_init[i]),2);
    }
    err_r = std::sqrt(err);
    ...
    for(i=0;i<N;i++)
    {
      err+=std::pow(std::abs(z[i]-z_init[i]),2);
    }
    err_z = std::sqrt(err);
    err = std::sqrt(err_r*err_r+err_z*err_z);
...
// b
  ...
  L = 0.;
    ...
    Output::print("Number of surface iterations: ", it);
...
// are dimentionless on V^1/3
  ...
  while(it<5 || err_r>eps || err_z>eps)
  {
	...
	{  
	  ...
	  H = std::sqrt(h_r[i]*h_r[i] + h_z[i]*h_z[i]);
      // for linear case
      f1[i] = H*H;
      f2[i] = Hn*Hn;
    ...
    if(it%500==0) Output::print("F");
    for(i=0;i<N+1;i++)
      {
	// for linear case
	f[i] = -W*L*f1[i]-W*L*hi*f2[i]; 
	if(it%500==0 && i==0) Output::print(z[i], ' ', f[i]+C);
	if(it%500==0 && i>0 && i!=N) Output::print((z[i-1]+z[i])/2., ' ', f[i]+C);
	if(it%500==0 && i==N) Output::print(z[N-1], ' ', f[N]+C);
	// for nonlinear case
	// f[i] = -2.*W*L/3./hi*f1[i]-W*L*f2[i];    
      }
//  ----------------------------------------------  
    ...
    //if(it%500==0) Output::print("C/L ", C/L, ' ', C, ' ', L);
...
// b
    ...
    if(adap)    
      b[N-1] = -h/2.*(f[N] + K/r[N-1] + C)*std::pow(ds[N-1],2);
    else
...
// are dimentionless on D^1/2  
  ...
  while(it<5 || err_beta>eps)
  {
    ...
    memcpy(beta_old,beta,N*sizeof(double));
    memcpy(r_old,r,N*sizeof(double));
    memcpy(z_old,z,N*sizeof(double));
              
    L = 0.;
    for(i=1;i<N-1;i++)
      L+=std::cos(beta_old[i])*z[i];
    L = std::pow(h*L+h*z[0]/2.,-1./2.)/2.;
  ...
  // for the case of magnetic saturation 
    /*
    memset(f1,0,N*sizeof(double));
    f2[0] = 1.; f2[N-1] = 0.;
    for(i=1;i<N-1;i++)
      f2[i] = std::pow(std::cos(beta_old[i]),2);
    */
  ...
  // taking into considaration the magnetic field
    for(i=0;i<N;i++)
    {
      ...
      {
        Hn = -std::sin(beta_old[i])*h_r[i] + 
             std::cos(beta_old[i])*h_z[i];
        H = std::sqrt(h_r[i]*h_r[i] + h_z[i]*h_z[i]);
      }
      // for linear case
       f1[i] = H*H;
 ...
 //  ----------------------------------------------  
    ...
    C = 0.;
    for(i=1;i<N-1;i++)
      C+=h*std::pow(r[i],K)*std::cos(beta_old[i])*f[i];
    C = -std::pow(2.,K)/r[N-1]*(1. + 
        (C+h*std::pow(r[0],K)*f[0]/2.-
         h*h/12.*(std::pow(r[N-1],K)*f[N-1]*F[N-1]-
         K*f[0]))/std::pow(r[N-1],K));
    

    F[0] = f[0] - K*(f[0]+C)/2. + C;
    for(i=1;i<N;i++)
      F[i] = f[i] - K*std::sin(beta_old[i])/r[i] + C;
      
  // df = -2WL/3/hi*df1-WL*df2
  // ----------------------------------------------
    memset(df1,0,N*sizeof(double)); // f1 not depend on s

  // for the case of magnetic saturation 
    /*
    df2[0] = 0.;
    for(i=1;i<N;i++)
      df2[i] = -std::sin(2.*beta_old[i])*F[i];
    */
  // taking into considaration the magnetic field
    for(i=0;i<N;i++)
    {
      ...
      // for linear case
      df2[i] = 2.*Hn*F[i]*
	(-std::cos(beta_old[i])*h_r[i] -
	 std::sin(beta_old[i])*h_z[i]);    
      // for nonlinear case
      // arg = H*gamma;
      // df2[i] = std::pow((1./std::tanh(arg)-1./arg)/H,2)*2.*Hn*F[i]*
      //         (-std::cos(beta_old[i])*h_r[i] -
      //          std::sin(beta_old[i])*h_z[i]);    
    }          
 ...
 //  ----------------------------------------------  
    ...
    dF[0] = 0.;
    for(i=1;i<N;i++)
      dF[i] = df[i] - K*(std::cos(beta_old[i])*F[i]*r[i]-
              std::sin(2.*beta_old[i])/2.)/r[i]/r[i];
              
    err_beta = 0.;
    for(i=1;i<N-1;i++)
    {
      err = std::abs((beta_old[i+1]-beta_old[i])/h-
...
//        i_err_beta = i;
  ...
  // r
    r[0] = 0.;
    for(i=1;i<N;i++)
      r[i] = r[i-1] + h*((std::cos(beta[i-1])+std::cos(beta[i]))/2.+
             h/12.*(F[i]*std::sin(beta[i])-F[i-1]*std::sin(beta[i-1])));

  // z
    z[N-1] = 0.;
    for(i=N-2;i>=0;i--)
      z[i] = z[i+1] - h*((std::sin(beta[i])+std::sin(beta[i+1]))/2.-
             h/12.*(F[i+1]*std::cos(beta[i+1])-F[i]*std::cos(beta[i])));
  ...
  *LL = L;
  ...
  Output::print("It = ", it, ", err_beta = ", err_beta);
...
// are dimentionless on V^1/3  
  ...
  while(it<5 || err_beta>eps) 
  {
    ...
    L = 0.;
    for(i=1;i<N-1;i++)
      L+=(h[i]+h[i+1])/2.*std::sin(beta_old[i])*r[i]*r[i];
	//-(h[i]*h[i]-h[i+1]*h[i+1])*(std::cos(beta_old[i])*r[i]*r[i]*F[i]+
	//r[i]*std::sin(2.*beta_old[i]))/12.;
    L = std::pow(-2.*M_PI*(L-h[N-1]*r[N-1]*r[N-1]/2.),-1./3.);
    
    if(it%500==0) Output::print("it=", it, " L=", L);
  // F = f - K*z'/r + C 
  // ----------------------------------------------
  ...
  // taking into considaration the magnetic field
    ...
    for(i=0;i<N;i++)
    {
      // for linear case
      f[i] = -W*L*f1[i]-W*L*hi*f2[i]; 
      //if(it%500==0) Output::print(z[i], ' ', -W*L*f1[i], ' ', -W*L*hi*f2[i], ' ', f[i]);
      // for nonlinear case
      // f[i] = -2.*W*L/3./hi*f1[i]-W*L*f2[i];    
    }
 //  ----------------------------------------------  
    ...
    C = 0.;
    for(i=1;i<N-1;i++)
      C+=(h[i]+h[i+1])/2.*std::pow(r[i],K)*std::cos(beta_old[i])*f[i];
	//-(h[i]*h[i]-h[i+1]*h[i+1])/12.*(K*std::cos(beta_old[i])*std::cos(beta_old[i])*f[i]-
	//std::pow(r[i],K)*std::sin(beta_old[i])*F[i]*f[i]+
	//std::pow(r[i],K)*std::cos(beta_old[i])*df[i]);
    C = -std::pow(2.,K)/r[N-1]*(1. + 
			   (C+h[1]/2.*std::pow(r[0],K)*f[0]
				//-1./12.*(h[N-1]*h[N-1]*std::pow(r[N-1],K)*f[N-1]*F[N-1]-h[1]*h[1]*K*f[0])
			    )/std::pow(r[N-1],K));    
    ...
    for(i=0;i<N;i++)      
      ...
      {
      if(i==0) 
	{Output::print(r[i], ' ', z[i], ' ', f[i], ' ', (f[0]+C)/2., ' ', F[i]);}
      else
	{Output::print(r[i], ' ', z[i], ' ', f[i], ' ', std::sin(beta_old[i])/r[i], ' ', F[i]);}
      }

    if(it%500==0)
      Output::print("F and beta'");
    for(i=0;i<N;i++)      
      if(it%500==0)
      {
      if(i==0) 
	{Output::print(r[i], ' ', z[i], ' ', F[i], ' ', (beta_old[1]-beta_old[0])/h[1]);}
      else
      {
	if(i==N-1)
	  {Output::print(r[i], ' ', z[i], ' ', F[i], ' ',
      (beta_old[N-1]-beta_old[N-2])/(h[N-1]));}
	else
	  {Output::print(r[i], ' ', z[i], ' ', F[i], ' ',
      (beta_old[i+1]-beta_old[i-1])/(h[i+1]+h[i]));}
      }
      }
    /*
	...
	{Output::print(r[i], ' ', z[i], ' ', F[i]);}
    */
    if(it%500==0) Output::print("beta(s)");
    for(i=0;i<N;i++)      
      if(it%500==0)
      {
      if(i==0) 
	{Output::print(beta_old[i], ' ', S(*a,t[i]));}
      else
	{Output::print(beta_old[i], ' ', S(*a,t[i]));}
      }
  // df = -2WL/3/hi*df1-WL*df2
  ...
  // taking into considaration the magnetic field
    ...
    //if(it%500==0) Output::print("df");
    for(i=0;i<N;i++)
    {
      // for linear case
      df[i] = -W*L*df1[i]-W*L*hi*df2[i]; 
      //if(it%500==0) Output::print(z[i], ' ', df[i]);
      // for nonlinear case
      // df[i] = -2.*W*L/3./hi*df1[i]-W*L*df2[i];    
    }
 //  ----------------------------------------------  
    ...
    for(i=1;i<N-1;i++)
    {
      if(up)
	{
	  err = std::abs((beta_old[i+1]-beta_old[i])/h[i+1]-
		    (F[i+1]+F[i])/2. 
		    //+ h[i+1]/12.*(dF[i+1]-dF[i])
		    );
	}
      else
	err = std::abs((beta_old[i]-beta_old[i-1])/h[i]-
		  (F[i-1]+F[i])/2. 
		  //+ h[i]/12.*(dF[i]-dF[i-1])
		  );
...
//        i_err_beta = i;
  ...
  // r
    r[0] = 0.;
    for(i=1;i<N;i++)
      r[i] = r[i-1] + h[i]*((std::cos(beta[i-1])+std::cos(beta[i]))/2.
			    //+h[i]/12.*(F[i]*std::sin(beta[i])-F[i-1]*std::sin(beta[i-1]))
			    );

  // z
    z[N-1] = 0.;
    for(i=N-2;i>=0;i--)
      z[i] = z[i+1] - h[i+1]*((std::sin(beta[i])+std::sin(beta[i+1]))/2.
			      //-h[i+1]/12.*(F[i+1]*std::cos(beta[i+1])-F[i]*std::cos(beta[i]))
			      );
 
    //if(it>1000000) {ID = 0; Output::print("Calculation was stoped"); break;}
    //if(it%100==0) Output::print(it, ' ', "curv = ", -(f[0]+C)/(2.*L));
    if(it%1000==0) Output::print("It = ", it, ", err_beta = ", err_beta);
  }

  Output::print("All iterations: ", it);
  Output::print("C=", C);
  //Output::print("beta'[0]-F[0] = ", (beta[1]-beta[0])/h[1]-F[0]);
  /*
  for(i=0;i<N;i++)
  {
    if(i!=0)
      Output::print(r[i], ' ', z[i], ' ', beta[i], ' ', F[i], ' ', std::sin(beta[i])/r[i], ' ', f[i]+C);
    else    
      Output::print(r[i], ' ', z[i], ' ', beta[i], ' ', F[i], ' ',  (f[0]+C)/2., ' ', f[i]+C);
  }
  */
  for(i=0;i<N;i++)
    {
...
// are dimentionless on V^1/3      
  ...
  // ----------------------------------------------  
    ...
    for(i=0;i<N;i++)
    {
      f[i] = -2.*W*L/3./hi*f1[i]-W*L*f2[i];    
      if(it%500==0) 
	Output::print(it, ' ', z[i], ' ', -2.*W*L/3./hi*f1[i], ' ', -W*L*f2[i], ' ', f[i]);
    }
 //  ----------------------------------------------  
    ...
    for(i=0;i<10;i++)
      ...
      {
	if(i==0) 
	  {Output::print(z[i], ' ', (f[0]+C)/2., ' ', F[i]);}
	else
	  {Output::print(z[i], ' ', std::sin(beta_old[i])/r[i], ' ', F[i]);}
      }
...
{
  return -a+2.*(a+1.)/(1.+std::pow(1.+2./a,1.-t));
}

double dSdt(double a, double t)
{
  return 2.*(a+1.)*std::pow(1.+2./a,1.-t)*std::log(1.+2./a)/std::pow(1.+std::pow(1.+2./a,1.-t),2);
}
...
double A(double curv, double h)
{
  ...
  // for plane case
  // c = std::pow(M_PI,1./2)*h/curv;
  // for axisymmetrical case
  c = std::pow(4.*M_PI/3.,1./3.)*h/curv;

  a_old = 0.; a_new = c;
 
  while(std::abs(a_new-a_old)>1.e-6)
    {
      a_old = a_new; 
      dsda = -1. + 2./(1.+std::pow(1+2./a_old,1.-h)) +
        4.*(a_old+1)*std::pow(1.+2./a_old,-h)*(1.-h)/
        (a_old*a_old*std::pow(1+std::pow(1+2./a_old,1.-h),2));
