...
int TBdCylinder::GetXYZofTS(double T, double S, double &X, double &Y,
                            double &Z) const
{
  double alpha = mRadius*std::cos(T);
  double beta  = mRadius*std::sin(T);
		   ...
		   double ax, double ay, double az, double nx, double ny, double nz)
{
  ...
  norm = std::sqrt(mAx*mAx + mAy*mAy + mAz*mAz);
  mAx /= norm; mAy /= norm; mAz /= norm;
  
  norm = std::sqrt(mNx*mNx + mNy*mNy + mNz*mNz);
  mNx /= norm; mNy /= norm; mNz /= norm;
  
  norm = std::sqrt(mBx*mBx + mBy*mBy + mBz*mBz);
