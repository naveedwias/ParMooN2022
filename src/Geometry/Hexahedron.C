...
{
  constexpr int HEXAMAXN_EpV = 3;
  constexpr int HEXAMAXN_VpF = 4;
  constexpr int HEXAMAXN_FpV = 3;
  constexpr int HEXAMAXN_EpF = 4;
  constexpr int HEXAMAXN_FpE = 2;
...
//  zc0 = (z1 + z3 + z4 - z0) * 0.5;
  ...
  detjk= std::abs(xc1*yc2*zc3 + xc2*yc3*zc1 + xc3*yc1*zc2 - xc3*yc2*zc1 - 
	      xc2*yc1*zc3 - xc1*yc3*zc2);

  // factor 2 = 8^(1/3) because of area of unit cube
  return(2*std::pow(detjk,1.0/3.0));
}
...
double THexahedron::GetMeasure(TVertex **Verts) const
{
  ...
  return std::abs(-x2*(y5*z4-y4*z5)+x5*(y2*z4-y4*z2)-x4*(y2*z5-y5*z2)+
    ...
    std::abs(-x2*(y3*z4-y4*z3)+x3*(y2*z4-y4*z2)-x4*(y2*z3-y3*z2)+
    ...
    std::abs(-x5*(y6*z7-y7*z6)+x6*(y5*z7-y7*z5)-x7*(y5*z6-y6*z5)+
    ...
    std::abs(-x5*(y7*z8-y8*z7)+x7*(y5*z8-y8*z5)-x8*(y5*z7-y7*z5)+
    ...
    std::abs(-x4*(y5*z7-y7*z5)+x5*(y4*z7-y7*z4)-x7*(y4*z5-y5*z4)+
	 x2*(y5*z7-y7*z5)-x5*(y2*z7-y7*z2)+x7*(y2*z5-y5*z2)+
...
Shapes THexahedron::CheckHexa(const TVertex * const * Vertices) const
{
  ...
  xt1 = std::abs( x0 - x1 - x3 + x2 + x4 - x5 - x7 + x6);
  xt2 = std::abs( x0 - x1 + x3 - x2 - x4 + x5 - x7 + x6);
  xt3 = std::abs( x0 + x1 - x3 - x2 - x4 - x5 + x7 + x6);
  xt4 = std::abs(-x0 + x1 + x3 - x2 + x4 - x5 - x7 + x6);

  yt1 = std::abs( y0 - y1 - y3 + y2 + y4 - y5 - y7 + y6);
  yt2 = std::abs( y0 - y1 + y3 - y2 - y4 + y5 - y7 + y6);
  yt3 = std::abs( y0 + y1 - y3 - y2 - y4 - y5 + y7 + y6);
  yt4 = std::abs(-y0 + y1 + y3 - y2 + y4 - y5 - y7 + y6);

  zt1 = std::abs( z0 - z1 - z3 + z2 + z4 - z5 - z7 + z6);
  zt2 = std::abs( z0 - z1 + z3 - z2 - z4 + z5 - z7 + z6);
  zt3 = std::abs( z0 + z1 - z3 - z2 - z4 - z5 + z7 + z6);
  zt4 = std::abs(-z0 + z1 + z3 - z2 + z4 - z5 - z7 + z6);
