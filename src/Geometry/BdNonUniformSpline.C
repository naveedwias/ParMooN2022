...
int TBdNonUniformSpline::GetTofXY(double X, double Y, double &T) const
{
  ...
  if(std::abs(Params[0]-X)<1e-8 && std::abs(Params[1]-Y)<1e-8) 
    {
      T = 0.0;
  ...
  for(i=0;i<N_Splines;i++) // (X,Y) coincides with a spline-point
    {
      ISpline = i*10;  
      if(std::abs(Params[ISpline+2]-X)<1e-8 && std::abs(Params[ISpline+3]-Y)<1e-8) 
	{
	  T = Params[ISpline+8];
	  //cout<<"GetTofXY T:"<<T<<endl;
	  return 0; 
  ...
  for(i=0;i<N_Splines;i++) // (X,Y) lies at the middle of a subspline
    {
      ISpline = i*10;  
      if(std::abs(GetLocalXofT(i+1,0.5)-X)<1e-8 && 
	 std::abs(GetLocalYofT(i+1,0.5)-Y)<1e-8) 
	{
	  if(i==0)
                                          ...
                                          double dx0, double dy0, double dx1, double dy1)
{
  ...
  for(i=1;i<=N_Splines;i++)
  {
    h[i] = std::sqrt((x[i]-x[i-1])*(x[i]-x[i-1])+(y[i]-y[i-1])*(y[i]-y[i-1]));
    t[i] = t[i-1] + h[i];
  }
