                                   ...
                                   double &tau, double &tauold, int &step_rej)
{
  ...
  switch(controller){
    case 0: // I Controller
      scale=tau*control_safty*std::pow(tol/err2,K_I);
      break;
    case 1: // PI controller
      scale=tau*control_safty*std::pow(tol/err2,K_I)*std::pow(err1/err2,K_P);
      break;
    case 2: // PID controller
      scale=tau*control_safty*std::pow(err1/err2,K_P)*std::pow(tol/err2,K_I)*std::pow((err1*err1)/(err2*err0),K_D);
      break;
    case 3: // PC controller
      scale=(tau*tau)/tauold*control_safty*std::pow(tol/err2,K_I)
              *std::pow(err1/err2,K_P);
      break;
  }
                                   ...
                                   int &step_rej)
{
  ...
  double tol=TDatabase::TimeDB->TIMESTEPLENGTH_TOL;
  acc=false;
  Output::print("KKP: ", K_P, "  KK_I:  ", K_I, "  :K_D:   ", K_D);
  switch(controller){
    case 0: // I Controller
      tau_star= old_time_step*control_safty
                * std::pow(tol/err_at_tn_p1,K_I);
      break;
    case 1: // PI controller
      tau_star=  old_time_step*control_safty
               * std::pow( (tol/err_at_tn_p1) , K_I)
               * std::pow( (err_at_tn/err_at_tn_p1),K_P);
      break;
    case 2: // PID controller
      tau_star= old_time_step * control_safty
                * std::pow( (err_at_tn/err_at_tn_p1), K_P)
                * std::pow( (tol/err_at_tn_p1), K_I)
                * std::pow( (err_at_tn*err_at_tn)/(err_at_tn_p1*err_at_tn_m1),K_D);
      break;
    case 3: // PC controller
      tau_star=(old_time_step*old_time_step)/old_time_step
               * control_safty*std::pow(tol/err_at_tn_p1,K_I)
               * std::pow( (err_at_tn/err_at_tn_p1),K_P);
      break;
  }
