...
//  q1_ndof=q1_space->get_n_dof();   // it is not used in the rest of this function
  q1_values=q1_function->GetValues();
//  q2_ndof=q2_space->get_n_dof();   // it is not used in the rest of this function
...
//  q1n_ndof=q1n_space->get_n_dof();  // not used in the rest of this function
...
//  p2_ndof=p2_space->get_n_dof();    // not used in the rest of this function
  ...
  for(j=0;j<k;j++)
  {
    ...
    auto dof=q1n_space->GetGlobalDOF(n0);
    for(l=0;l<5;l++)
      fine_values[l]=q1n_values[dof[l]];

    dof=q1n_space->GetGlobalDOF(n1);
