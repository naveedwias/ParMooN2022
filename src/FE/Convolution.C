...
double CharacteristicFilterWidth(double h)
{
  ...
  if (filter_power==1)
    ...
    delta =  filter_constant*std::pow(h,filter_power);
...
double GaussianFilter(double delta, double dist_sq)
{
  ...
  a = (gamma/M_PI)*std::exp(-gamma * dist_sq/(delta*delta))/(delta*delta);
  return(a);
}
...
{
  int i,j,k,l;
  int N_Cells, N_Points, N_, N_U, N_loc_dofConv;
  int UsedNeigh[N_FEs2D];
  int neigh_i, N_Neigh, ll, lll, N_PointsNeigh, N_Edges, same_dof;
  FE_type CurrentElementNeigh,LocalUsedElementsNeigh[N_FEs2D] ;
  ...
  MultiIndex2D NeededDerivatives[1] = { MultiIndex2D::D00 };
...
// ########################################################################
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    std::vector<const FiniteElement*> used_fe(1, &fespace->get_fe(i));
    ...
    FEDatabase::GetOrigFromRef(RefTrans,N_loc_dofConv, xi_ref, eta_ref,
                               X_orig, Y_orig);
    ...
    auto DOFConv = fespaceConv->GetGlobalDOF(i);
    ...
    for(l=0;l<N_loc_dofConv;l++)         
    {
      ...
      for(j=0;j<N_Points;j++) 
      {
        ...
        // in original cell
        distance_sq = ((X_orig[l]-p.x)*(X_orig[l]-p.x)+
                       (Y_orig[l]-p.y)*(Y_orig[l]-p.y)); 
        ...
        value += g*weight*FEValue_u1_QuadPoint[j];
        value1 += g*weight*FEValue_u2_QuadPoint[j];
      }
      ...
      for(ll=0;ll<N_Edges;ll++)                           
      {
        ...
        // get finite element in neigh mesh cell wrt the
        // finite element space of the convolution
        auto neighbor_fe = fespaceConv->get_fe(neigh_i);
        
        // # of basis functions, is the same as N_loc_dof
        N_Neigh = neighbor_fe.GetN_DOF();
     
        // find the part of the global index array where the information
        // for the neigh mesh cell are stored: DOF
        auto DOFNeigh = fespaceConv->GetGlobalDOF(neigh_i);
        ...
        neighbor_fe = fespace->get_fe(neigh_i);
        ...
        // find the part of the global index array where the information
        // for the neigh mesh cell are stored: DOF
        DOFNeigh = fespace->GetGlobalDOF(neigh_i);

        // initialize auxiliary array UsedNeigh
        memset(UsedNeigh, 0, N_FEs2D*sizeof(int));
        
        // get id of finite element space in neigh cell 
        CurrentElementNeigh = fespace->get_fe_type(neigh_i);
        ...
        memset(LocalUsedElementsNeigh, 0, sizeof(int)*N_FEs2D);
        j = 0;
        for(k=0;k<N_FEs2D;k++)
          if(UsedNeigh[k])
          {
            LocalUsedElementsNeigh[j] = ( FE_type )k;
            j++;
          }
        ...
        std::vector<const FiniteElement*> used_fe_neight(1, &fespace->get_fe(neigh_i));
        FEDatabase::GetOrig(used_fe_neight, Coll, neigh, SecondDer,
                            qf_ref_neigh, qf_orig_neigh);
        N_PointsNeigh = qf_orig_neigh.GetN_QuadPoints();
        ...
        for(k=0;k<N_Derivatives;k++)
        {
          // get fe values of the basis functions in the quad. points
          // in the current element (pointer to pointer)
          OrigFEValuesNeigh = FEDatabase::GetOrigElementValues(
            *neighbor_fe.GetBaseFunct(), NeededDerivatives[k]);
          // accumulate the values of the fe function 
          // do it for all quad. points
        ...
        for(j=0;j<N_PointsNeigh;j++) 
        { 
          auto p = qf_orig_neigh.get_point(j);
          // square of the distance between local dof and quad. point
          // in original cell 
          switch(TDatabase::ParamDB->INTERNAL_PERIODIC_IDENTITY)
          {
            ...
            case 2:
              // periodic square (1,1)^2 
                if (std::abs(X_orig[l]-p.x)> 1.5)
                {
                  if (X_orig[l]-p.x>1.5)
                    distance_sq = ((X_orig[l]-p.x-2)*(X_orig[l]-p.x-2)+
                                   (Y_orig[l]-p.y)*(Y_orig[l]-p.y)); 
                  else
                    distance_sq = ((X_orig[l]-p.x+2)*(X_orig[l]-p.x+2)+
                                   (Y_orig[l]-p.y)*(Y_orig[l]-p.y));                                       
          ...
          // input: delta, distance_sq
          g = GaussianFilter(delta,distance_sq);
          //cout << "dist " <<  std::sqrt(distance_sq) << endl;
          //
          // apply quadrature rule in ref. cell 
          double weight = qf_orig_neigh.get_weight(j);
          value += g * weight * FEValue_u1_QuadPointNeigh[j];
          value1 += g * weight * FEValue_u2_QuadPointNeigh[j];
...
//  for (i=0;i<N_DOFConv; i++)
  //{
//    Output::print(i, " x ", x_conv[i], " y  ", y_conv[i], " u_conv ", u_conv[i]);
  //}
...
void  ConvoluteVelocityFull(TFEVectFunct2D *u, TFEVectFunct2D *uConv)
{
  int i,j,k,l;
  int N_Cells, N_Points, N_, N_U, N_loc_dofConv;
  int neigh_i, N_Neigh, ll, lll, N_PointsNeigh;
  const double *xi_ref, *eta_ref;
  double X_orig[MaxN_PointsForNodal2D], Y_orig[MaxN_PointsForNodal2D];
  ReferenceTransformation_type RefTrans;
  double FEValue_u1_QuadPoint[MaxN_QuadPoints_2D];
  ...
  memset(conv_comp,0,N_UConv*sizeof(int));
...
// ########################################################################
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    {
      LocalUsedElements[i] = &fespace->get_fe(i);
    }
    ...
    for(l=0;l<N_loc_dofConv;l++)         
    {
      ...
      for(ll=0;ll<N_Cells;ll++)                           
      {
        ...
        switch(TDatabase::ParamDB->INTERNAL_PERIODIC_IDENTITY)
        {
          ...
          case 2:
            // periodic square (1,1)^2 
            if (std::abs(X_orig[l]-x_vertex)> 1.5)
            {
        ...
        // get id of finite element in neigh mesh cell wrt the finite
        // element space of the velocity
        auto neighbor_fe = fespace->get_fe(neigh_i);
        ...
        // find the part of the global index array where the information
        // for the neigh mesh cell are stored: DOF
        auto DOFNeigh = fespace->GetGlobalDOF(neigh_i);
        ...
        //          xiNeigh, etaNeigh, weightsNeigh, XNeigh, YNeigh,  
        //          AbsDetjkNeigh are pointers to arrays
        std::vector<const FiniteElement*> LocalUsedElementsNeigh(1, &neighbor_fe);
        FEDatabase::GetOrig(LocalUsedElementsNeigh, Coll, neigh, SecondDer,
                            qf_ref_neigh, qf_orig_neigh);
        N_PointsNeigh = qf_orig_neigh.GetN_QuadPoints();
...
{
  int i,j,k,l;
  int N_Cells, N_Points, N_, N_U, N_DOF, N_loc_dofConv;
  int neigh_i, N_Neigh, ll, lll, N_PointsNeigh, N_Edges, same_dof;
  const double *xi_ref, *eta_ref;
  double X_orig[MaxN_PointsForNodal2D], Y_orig[MaxN_PointsForNodal2D];
  ReferenceTransformation_type RefTrans;
  ...
  int N_Derivatives = 2;
  MultiIndex2D NeededDerivatives[2] = { MultiIndex2D::D10, MultiIndex2D::D01 };
...
// ########################################################################
  ...
  // second part of the array : u2
  Values = u->GetValues();           
  Output::print("U ", std::sqrt(Ddot(N_DOF, Values, Values)));
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    // find local used finite elements on this cell
    // ####################################################################
    std::vector<const FiniteElement*> LocalUsedElements(1, &fespace->get_fe(i));
    // ####################################################################
    // calculate values on original element
    // ####################################################################
    ...
    // known. Now, get information in the original mesh cell. 
    // Every finite element gets automatically a quadrature rule.
    // input: LocalUsedElements, cell, SecondDer
    // output : N_Points - number of quadrature points
    //          xi - xi-values of quad. points in ref. cell
    //          eta - eta-values of quad. points in ref. cell
    //          weigths - weights fro quad. rule in quad. points 
    ...
    // get id of finite element in current mesh cell
    // get fe from its id
    auto Element = fespaceConv->get_fe(i);
    // get reference transformation 
    RefTrans = Element.GetRefTransID();
    // get pointer to the nodal functionals (fe nodes) of the fe 
    // (in ref. cell)
    ...
    for(l=0;l<N_loc_dofConv;l++)         
    {
      ...
      for(ll=0;ll<N_Edges;ll++)                           
      {
        ...
        FEDatabase::GetOrig(LocalUsedElementsNeigh, Coll, neigh, SecondDer,
                            qf_ref_neigh, qf_orig_neigh);
        N_PointsNeigh = qf_orig.GetN_QuadPoints();
  ...
  Output::print("C0 ", std::sqrt(Ddot(N_UConv, u_conv, u_conv)));
  Output::print("C1 ", std::sqrt(Ddot(N_UConv, u_conv+N_UConv, u_conv+N_UConv)));
  Output::print("C2 ", std::sqrt(Ddot(N_UConv, u_conv+2*N_UConv, u_conv+2*N_UConv)));
  Output::print("C3 ", std::sqrt(Ddot(N_UConv, u_conv+3*N_UConv, u_conv+3*N_UConv)));
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    for(l=0;l<N_loc_dofConv;l++)         
    {
      ...
      for(ll=0;ll<N_Cells;ll++)                           
      {
        ...
        // get finite element in neigh mesh cell wrt the finite
        // element space of the velocity
        auto neighbor_fe = fespace->get_fe(neigh_i);
...

{
  ...
  //BaseFuncts = FEDatabase::GetBaseFunct2D_IDFromFE2D();
  // gives the number of available basis functions from the
  // fedatabase
  //N_BaseFunct = FEDatabase::GetN_BaseFunctFromFE2D();
...
// ########################################################################
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    FEDatabase::GetOrig(N_LocalUsedElements, LocalUsedElements, 
                        cell, SecondDer,
    ...
    // get id of finite element in current mesh cell
    CurrentElement = fespace->get_fe_type(i);
    CurrentElementConv = fespaceConv->get_fe_type(i);
    // get fe from its id
    auto Element = fespaceConv->get_fe(i);
    // get reference transformation 
    RefTrans = Element.GetRefTransID();
    ...
    // output : X_orig, Y_orig - pointers to arrays with coordinates
    //          AbsDetjk - same as above    
    FEDatabase::GetOrigFromRef(RefTrans,N_loc_dofConv, xi_ref, 
                               eta_ref, X_orig, Y_orig);
    ...
    DOFConv = fespaceConv->GetGlobalDOF(i);
    // copy the values of the finite element function from the global
    // array (Values) to local arrays 
    for(l=0;l<N_;l++)
    {
    ...
    for(l=0;l<N_loc_dofConv;l++)         
    {
      ...
      for(ll=0;ll<N_Cells;ll++)                           
      {
        ...
        FEDatabase::GetOrig(N_LocalUsedElementsNeigh, LocalUsedElementsNeigh, 
                            neigh, SecondDer,
        ...
        for(k=0;k<N_Derivatives;k++)
        {
          ...
          // in the current element (pointer to pointer)
          OrigFEValuesNeigh = FEDatabase::GetOrigElementValues(
            BaseFunctNeigh, NeededDerivatives[k]);
          // accumulate the values of the fe function 
          // do it for all quad. points
          for(j=0;j<N_PointsNeigh;j++)
          {
        ...
        { 
          ...
          switch(TDatabase::ParamDB->INTERNAL_PERIODIC_IDENTITY)
          {
            ...
            case 2:
              // periodic square (1,1)^2 
                if (std::abs(X_orig[l]-XNeigh[j])> 1.5)
                {
...
//  double LocError[4];
  ...
  // defined in NavierStokes.h 
  // here we get as result {D00}
  MultiIndex3D NeededDerivatives[1] = { MultiIndex3D::D000 };
...
// ########################################################################
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    FEDatabase::GetOrig({&CurrentElement}, Coll, cell, SecondDer, qf_ref,
                        qf_orig);
    N_Points = qf_orig.GetN_QuadPoints();
    ...
    // compute geometric positions of the fe nodes
    // get fe from its id
    auto ElementConv = fespaceConv->get_fe(i);;
    // get reference transformation 
    auto RefTrans = ElementConv.GetRefTransID();
    // get pointer to the nodal functionals (fe nodes) of the fe 
    // (in ref. cell)
    auto nf = ElementConv.GetNodalFunctional();
    // get number and coordinates of local dof in ref cell
    ...
    // output : X_orig, Y_orig - pointers to arrays with coordinates
    //          AbsDetjk - same as above    
    FEDatabase::GetOrigFromRef(RefTrans,N_loc_dofConv,
    ...
    for(l=0;l<N_loc_dofConv;l++)         
    {
      ...
      for(j=0;j<N_Points;j++) 
      {
        ...
        value += g * weight * FEValue_u1_QuadPoint[j];
        value1 += g * weight * FEValue_u2_QuadPoint[j];
        value2 += g * weight * FEValue_u3_QuadPoint[j];
      }
      // add local result to global array
      u_conv[index] += value;
      u_conv[index+N_UConv] += value1;
      ...
      for(ll=0;ll<N_Faces;ll++)                           
      {
        ...
        // get id of finite element in neigh mesh cell wrt the
        // finite element space of the convolution
        auto CurrentElementNeigh = fespaceConv->get_fe(neigh_i);
 
        // # of basis functions, is the same as N_loc_dof
        N_Neigh = CurrentElementNeigh.GetN_DOF();
        ...
        // get id of finite element in neigh mesh cell wrt the finite
        // element space of the velocity
        CurrentElementNeigh = fespace->get_fe(neigh_i);
        
        // get pointer to basis functions of the neighbour element
        auto& bf_neigh = *CurrentElementNeigh.GetBaseFunct();
        ...
        FEDatabase::GetOrig({&CurrentElementNeigh}, Coll, neigh, SecondDer,
                            qf_ref_neigh, qf_orig_neigh);
        N_PointsNeigh = qf_orig_neigh.GetN_QuadPoints();
        ...
        for(j=0;j<N_PointsNeigh;j++) 
        {
          ...
          value += g * weight * FEValue_u1_QuadPointNeigh[j];
          value1 += g * weight * FEValue_u2_QuadPointNeigh[j];
          value2 += g * weight * FEValue_u3_QuadPointNeigh[j];
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    for(l=0;l<N_loc_dofConv;l++)         
    {
      ...
      for(ll=0;ll<N_Cells;ll++)                           
      {
        ...
        // get id of finite element in neigh mesh cell wrt the finite
        // element space of the velocity
        auto CurrentElementNeigh = fespace->get_fe(neigh_i);
...
//  double LocError[4];
  ...
  bool SecondDer[1] = { false };
  int N_Derivatives = 3;
  MultiIndex3D NeededDerivatives[3] = { MultiIndex3D::D100, MultiIndex3D::D010,
                                        MultiIndex3D::D001 };
  //TDatabase::ParamDB->INTERNAL_QUAD_RULE=1; there is something wrong !!!!
        ...
        for(j=0;j<N_PointsNeigh;j++) 
        {
          ...
          switch(TDatabase::ParamDB->INTERNAL_PERIODIC_IDENTITY)
          {
            ...
            case 1:
              ...
              if ((std::abs(X_orig[l]-p.x)> 1.5)||(std::abs(Z_orig[l]-p.z)>1.5))
              {
                 // Output::print("points ", X_orig[l], " ", Y_orig[l], " ", Z_orig[l], 
                 //       " ; ", p.x, " ", p.y, " ", p.z);
                if (X_orig[l]-p.x>1.5)
                  distance_sq = ((X_orig[l]-p.x-2)*(X_orig[l]-p.x-2)+
                                 (Y_orig[l]-p.y)*(Y_orig[l]-p.y)+
                ...
                if (Z_orig[l]-p.z>1.5)
                  distance_sq = ((X_orig[l]-p.x)*(X_orig[l]-p.x)+
                                 (Y_orig[l]-p.y)*(Y_orig[l]-p.y)+
                                 (Z_orig[l]-p.z-2)*(Z_orig[l]-p.z-2)); 
                if (Z_orig[l]-p.z<-1.5)
                  distance_sq = ((X_orig[l]-p.x)*(X_orig[l]-p.x)+
                                 (Y_orig[l]-p.y)*(Y_orig[l]-p.y)+
                                 (Z_orig[l]-p.z+2)*(Z_orig[l]-p.z+2)); 
                //Output::print(distance_sq);
              }
              else
                distance_sq = ((X_orig[l]-p.x)*(X_orig[l]-p.x)+
                               (Y_orig[l]-p.y)*(Y_orig[l]-p.y)+
              ...
              break;
            default :
              ErrThrow("Not implemented for this INTERNAL_PERIODIC_IDENTITY !!!");
