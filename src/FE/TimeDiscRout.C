...
void SetTimeDiscParameters(int increase_count)
{
  ...
  static double theta=1-std::sqrt(double(2.0))/2;
  ...
  switch(TDatabase::TimeDB->TIME_DISC)
  {
    ...
    case 3: // FRACTIONAL_STEP WITH CANONICAL RHS
      switch(count)
      {
        ...
        break;
      default:
          ErrThrow("Pass 1 as the argument in SetTimeDiscParameters() for FRACTIONAL_STEP scheme; !!!");
          break;
      }
      if(count==3) count=0;
    ...
    case 5: // Rosenbrock methods
      switch(rb_type)
      {
        ...
        case 24: // ROSI2PW
          ...
          break; // ROSI2Pw
          default:
            ErrThrow("This Rosenbrock-scheme is not implemented");
          break;
      }
      TDatabase::TimeDB->CURRENTTIMESTEPLENGTH =
        TDatabase::TimeDB->TIMESTEPLENGTH;
      ...
      switch(rb_type)
      {
        ...
        case 3: // fractional-step-theta-scheme (pressure corrected)
          ...
          b4 = (4*std::sqrt(2.0) - 5)/(std::sqrt(2.0)+1);
          b4 = (4*std::sqrt(2.0) - 5)/3/(std::sqrt(2.0)+1);
          ...
          switch(count)
          {
            case 1: // 1. substep
              ...
              TDatabase::TimeDB->RB_MS_I     =     1 - b4 - (2 - 5*std::sqrt(2.0)/4 + b4*std::sqrt(2.0)/2) - (2 - std::sqrt(2.0) - b4);
            break;
            case 2: // 2. substep
              ...
              TDatabase::TimeDB->RB_MS_I     =     2 - std::sqrt(2.0) - b4;
            break;
            case 3: // 3. substep
              TDatabase::TimeDB->RB_A_IJ[0]  =     theta*beta;
              ...
              TDatabase::TimeDB->RB_MS_I     =     2 - 5*std::sqrt(2.0)/4 + b4*std::sqrt(2.0)/2;
            break;
            case 4: // 4. substep
              TDatabase::TimeDB->RB_A_IJ[0]  =     theta*beta;
              TDatabase::TimeDB->RB_A_IJ[1]  =     (1-theta)*alpha;
        ...
        case 4: // DIRK3
          ...
          switch(count)
          {
            case 1: // 1. substep
              ...
              TDatabase::TimeDB->RB_MS_I     =     5.0/12 + std::sqrt(3.0)/12;
            break;
            case 2: // 2. substep
              ...
              TDatabase::TimeDB->RB_MS_I     =     3.0/4 + std::sqrt(3.0)/12;
            break;
            case 3: // 3. substep
              ...
              TDatabase::TimeDB->RB_MS_I     =     -1.0/6 - std::sqrt(3.0)/6;
            break;
          }
          if (count==3)  count = 0;
          break; // DIRK3
        case 5: // DIRK3L
          gamma = 1.0 - std::sqrt(2.0)/2;
          switch(count)
          {
            case 1: // 1. substep
              TDatabase::TimeDB->RB_A_IJ[0]  =     0.0;
              ...
              TDatabase::TimeDB->RB_MS_I     =     0.5 - std::sqrt(2.0)/8;
            break;
            case 2: // 2. substep
              TDatabase::TimeDB->RB_A_IJ[0]  =     gamma;
              TDatabase::TimeDB->RB_A_IJ[1]  =     gamma;
            ...
            case 3: // 3. substep
              ...
              TDatabase::TimeDB->RB_MS_I     =     std::sqrt(2.0)/4;
            break;
          }
          if (count==3)  count = 0;
          break; // DIRK3L
        ...
        case 11: // Alexander: SDIRK(3,2) - order 3
          alpha = 1 - std::sqrt(2.0)/2*std::cos(1.0/3*atan(std::sqrt(2.0)/4)) + std::sqrt(6.0)/2*std::sin(1.0/3*atan(std::sqrt(2.0)/4));
...
int GetN_SubSteps()
{
  ...
  switch(TDatabase::TimeDB->TIME_DISC)
  {
    ...
    case 5:
      switch(TDatabase::TimeDB->RB_TYPE)
      {
        ...
        case 24:
          ...
          break;
        default:
          ErrThrow("This RB-TYPE does not exist!");
          break;
      }
    break;
    case 6:
    switch(TDatabase::TimeDB->RB_TYPE)
      {
        ...
        case 15:
          ...
          break;
        default:
          ErrThrow("This DIRK-TYPE (", TDatabase::TimeDB->RB_TYPE, ") does not exist!");
          break;
        }
        break;
    case 7:
    case 8:
      ...
      break;
      default:
          ErrThrow("Time discretization not implemented !!!");
          break;
  } // endswitch
