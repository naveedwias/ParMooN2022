...
void TTetraAffin::GetOrigFromRef(double xi, double eta, double zeta, double &X,
...
void TTetraAffin::GetTransformationDerivatives(double, double, double,
  double* matrix) const
{
  matrix[0] = xc1;
  matrix[1] = yc1;
...
void TTetraAffin::GetOrigFromRef(const TQuadFormula& qf_ref,
...
void TTetraAffin::GetRefFromOrig(double X, double Y, double Z, double &xi,
                ...
                int _BaseVectDim) const
{
  if(_BaseVectDim == 1)
  {
    ...
    for(int i=0;i<N_BaseFunct;i++)
    {
      uxorig[i] = i11 * uxiref[i] + i12 * uetaref[i] + i13 * uzetaref[i];
      uyorig[i] = i21 * uxiref[i] + i22 * uetaref[i] + i23 * uzetaref[i];
      uzorig[i] = i31 * uxiref[i] + i32 * uetaref[i] + i33 * uzetaref[i];
    } // endfor i
  }
  else
  {
...
void TTetraAffin::GetOrigAllDerivatives(double xi, double eta, double zeta,
                                        int N_BaseFunct,
